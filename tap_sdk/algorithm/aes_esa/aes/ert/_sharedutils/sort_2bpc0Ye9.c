...
#include "sort_2bpc0Ye9.h"

/* Function for MATLAB Function: '<S955>/MATLAB Function' */
void sort_2bpc0Ye9(real_T x[96])
{
  real_T xwork[96];
  real_T x4[4];
  int32_T idx[96];
  ...
  memset(&xwork[0], 0, 96U * sizeof(real_T));
  nNaNs = -95;
  ib = 0;
  for (m = 0; m < 96; m++) {
    if (rtIsNaN(x[m])) {
  ...
  if (1 - nNaNs > 1) {
    ...
    while (ib > 1) {
      if ((ib & 1U) != 0U) {
        ...
        i2 = 1 - (nNaNs + i1);
        if (i2 > m) {
          merge_ZXWlctnw(idx, x, i1, m, i2 - m, iwork, xwork);
        }
      }
      ...
      ib >>= 1;
      for (i2 = 0; i2 < ib; i2++) {
        merge_ZXWlctnw(idx, x, i2 * i1, m, m, iwork, xwork);
      }
    ...
    if (1 - nNaNs > m) {
      merge_ZXWlctnw(idx, x, 0, m, 1 - (nNaNs + m), iwork, xwork);
    }
  }
}
