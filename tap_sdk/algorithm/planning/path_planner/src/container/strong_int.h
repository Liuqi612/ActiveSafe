...
class StrongInt {
 public:
    ...
    StrongInt &operator--() {
        ValidatorType::template ValidateSubtract<ValueType>(value_,
                                                            ValueType(1));
        --value_;
        return *this;
    }
    ...
    StrongInt &operator*=(ArgType arg) {
        ValidatorType::template ValidateMultiply<ValueType, ArgType>(value_,
                                                                     arg);
        value_ *= arg;
        return *this;
    }
...
#define DEFINE_STRONG_INT_TYPE(type_name, value_type)                         \
    ...
    typedef pnc_x::intops::StrongInt<type_name##_strong_int_tag_, value_type, \
                                     pnc_x::intops::NullStrongIntValidator>   \
        type_name;

#define STRONG_INT_VS_STRONG_INT_BINARY_OP(op)                              \
    template <typename TagType, typename ValueType, typename ValidatorType> \
    inline pnc_x::intops::StrongInt<TagType, ValueType, ValidatorType>      \
    operator op(                                                            \
        pnc_x::intops::StrongInt<TagType, ValueType, ValidatorType> lhs,    \
        pnc_x::intops::StrongInt<TagType, ValueType, ValidatorType> rhs) {  \
        lhs op## = rhs;                                                     \
        return lhs;                                                         \
    }
...
struct hash<pnc_x::intops::StrongInt<Tag, Value, Validator>> {
    size_t operator()(
        const pnc_x::intops::StrongInt<Tag, Value, Validator> &idx) const {
        return static_cast<size_t>(idx.value());
    }
};

template <typename Tag, typename Value, typename Validator>
std::string to_string(pnc_x::intops::StrongInt<Tag, Value, Validator> i) {
