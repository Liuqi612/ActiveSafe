...
inline void LerpTrajPoint(const ApolloTrajectoryPointProto &p0,
                          const ApolloTrajectoryPointProto &p1,
                          double alpha,
...
inline ApolloTrajectoryPointProto QueryApolloTrajectoryPointByS(
    ...
    ApolloTrajectoryPointProtoIterator end,
    double s) {
    const auto it = std::lower_bound(
        begin, end, s,
        [](const auto &p, double s) { return p.path_point().s() < s; });
...
std::vector<ApolloTrajectoryPointProto> ToApolloTrajectoryPointProto(
    absl::Span<const ::pnc_x::prediction::PredictedTrajectoryPoint> traj);
