...
using ResampledObjectsHistory = std::vector<std::vector<pnc_x::ObjectProto>>;
...
struct ObjectMotionState {
    ...
    std::string DebugString() const {
        return absl::StrFormat(
            "timestamp: %.6f, pos: %s, heading: %.6f, vel: %s.", timestamp,
            pos.DebugString(), heading, vel.DebugString());
    }
};
...
struct AgentCentricObjectOut {
    ...
    boost::optional<double> startup_prob;
};
...
struct AVObjectRelation {
    ...
    std::string DebugString() const {
        return absl::StrFormat("void: %f, yield: %f, pass: %f", no_relation,
                               yield, pass);
    }
};
