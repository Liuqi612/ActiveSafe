...
absl::Status AlignPerceptionObjectTime(double current_time,
    ...
    if (time_diff < 0.0) {
        return absl::InvalidArgumentError(absl::StrFormat(
            "Provided time[%f] is less than perception's time[%f]",
    ...
    const auto contour_points = TransformPoints(
        object->contour(), pos, pos_shift, rotation.x(), rotation.y());
