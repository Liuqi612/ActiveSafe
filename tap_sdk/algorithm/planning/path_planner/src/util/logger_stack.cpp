...
LoggerSegment *LoggerSegment::SetName(const std::string &new_name) {
    _name = new_name;
    return this;
}

LoggerSegment *LoggerSegment::SetStartTime(const int64_t &new_start_time) {
...
LoggerSegment *LoggerStack::StartSegment(const int64_t &time) {
    if (_current_stack.empty()) {
        ...
        (void)_root_segment->SetStartTime(time);
...
void LoggerStack::EndSegment(const int64_t &time,
    ...
    if (!XCHECK(!_current_stack.empty())) {
        return;
    }
...
void LoggerStack::EndSegment(const int64_t &time) {
    if (!XCHECK(!_current_stack.empty()) ||
        !XCHECK(!_current_stack.back()->name().empty())) {
        return;
    }
