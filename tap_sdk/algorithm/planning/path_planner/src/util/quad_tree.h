...
class QuadTree {
 ...
 public:
    ...
    class iterator {
     public:
        ...
        explicit iterator(std::vector<ElementInfo> *elements, int id)
            : elements_(XCHECK_NOTNULL(elements)), id_(id) {}
     ...
     private:
        bool IsValid() const {
            return (elements_ != nullptr) && (id_ >= 0) &&
                   (id_ < elements_->size());
        }
    ...
    class const_iterator {
     public:
        ...
        explicit const_iterator(const std::vector<ElementInfo> *elements,
                                int id)
            : elements_(XCHECK_NOTNULL(elements)), id_(id) {}
    ...
    explicit QuadTree(int local_depth)
        : max_num_elements_(INT_MAX / local_depth - 5), depth_(local_depth) {
        XCHECK_GT(depth_, 0);
        XCHECK_LE(depth_, kMaxDepth);
    ...
    bool AddElement(int level, int xid, int yid, T element) {
        ...
        for (int l = 0; l <= level; ++l) {
            ...
            if (l == level) {
                XCHECK_LT(nodes_[node].element_info_id, 0);
    ...
    const T *GetElement(int level, int xid, int yid) const {
        ...
        return element_id >= 0 ? &(element_infos_[element_id].element)
                               : nullptr;
    }
    ...
    const T *GetElementByPosition(double x,
                                  double y,
    ...
    T *GetMutableElementByPosition(double x,
                                   double y,
                                   double root_box_width = 1.,
    ...
    ElementReferenceArray GetMutableElementsByPosition(
        double x,
        double y,
        double root_box_width = 1.,
        int max_level = INT_MAX) {
 ...
 private:
    ...
    int ElementInfoIdFromPosition(double x,
                                  double y,
                                  int max_level,
