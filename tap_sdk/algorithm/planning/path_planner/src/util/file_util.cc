...
absl::Status RemoveFile(std::string filepath) {
    std::error_code ec;
    const bool success = filesystem::remove(filepath);
...
absl::Status CreateDirectory(std::string dir) {
    std::error_code ec;
    const bool success = filesystem::create_directories(dir);
    if (success || (ec.value() == 0)) {
        return absl::OkStatus();
    } else {
        return absl::UnknownError(ec.message());
...
absl::Status RemoveDirectory(std::string dir) {
    std::error_code ec;
    const auto count = filesystem::remove_all(dir);
...
absl::StatusOr<std::vector<std::string>> ListDirectory(std::string dir) {
    ...
    return absl::StatusOr<std::vector<std::string>>(result);
}
...
absl::StatusOr<std::vector<std::string>> ListDirectoryRecursive(
    ...
    for (const auto &entry :
        ...
        result.push_back(filesystem::relative(entry.path(), base_path).c_str());
    }
    return absl::StatusOr<std::vector<std::string>>(result);
}
...
bool TextFileToProto(const std::string &file_name,
    ...
    if (fd < 0) {
        // LOG(ERROR) << "Failed to open " << file_name << " in text mode: "
...
bool FileToProto(const std::string &filename,
                 google::protobuf::Message *proto) {
    return BinaryFileToProto(filename, proto) ||
