...
void InsertOrDie(Collection *const collection,
    ...
    XCHECK(InsertIfNotPresent(collection, value));
}
...
void InsertOrDie(Collection *const collection,
    ...
    XCHECK(InsertIfNotPresent(collection, key, data));
}
...
typename Collection::value_type::second_type &LookupOrInsertNew(
    ...
    std::pair<typename Collection::iterator, bool> ret =
        collection->insert(typename Collection::value_type(
            key, static_cast<typename Collection::value_type::second_type>(
                     nullptr)));
    if (ret.second) {
...
void InsertKeysFromMap(const MapContainer &map_container,
    ...
    if (!XCHECK(key_container != nullptr)) {
        return;
    }
...
void AppendValuesFromMap(const MapContainer &map_container,
    ...
    if (!XCHECK(value_container != nullptr)) {
        return;
    }
    for (typename MapContainer::const_iterator it = map_container.begin();
         it != map_container.end(); ++it) {
