...
class Vec2 : public Eigen::Matrix<T, 2, 1> {
 public:
    ...
    Vec2() : Base(0, 0) {}
    Vec2(T local_x, T local_y) : Base(local_x, local_y) {}
    explicit Vec2(Vec3<T> v) : Base(v.x(), v.y()) {}

    static constexpr Vec2<T> Zero() { return Vec2<T>(T(0), T(0)); }

    template <typename OtherDerived>
    Vec2(const Eigen::EigenBase<OtherDerived> &vec) : Base(vec) {}

    explicit Vec2(const Vec2dProto &proto) : Base(proto.x(), proto.y()) {}

    void set_x(T local_x) { this->x() = local_x; }
    void set_y(T local_y) { this->y() = local_y; }
    ...
    template <typename BuiltinT,
              typename std::enable_if<std::is_arithmetic<BuiltinT>::value,
                                      bool>::type = 0>
    Vec2<T> operator+(BuiltinT rhs) const {
        return {this->x() + rhs, this->y() + rhs};
    }
    template <typename OtherT,
              typename std::enable_if<!std::is_arithmetic<OtherT>::value,
                                      bool>::type = 0>
    ...
    template <typename OtherT,
              typename std::enable_if<!std::is_arithmetic<OtherT>::value,
                                      bool>::type = 0>
    decltype(std::declval<Base>().operator*(std::declval<OtherT>())) operator*(const OtherT &rhs) const {
        return this->Base::operator*(rhs);
    }
    template <typename BuiltinT,
              typename std::enable_if<std::is_arithmetic<BuiltinT>::value,
    ...
    T DistanceTo(const Vec2<T> &vec) const { return (*this - vec).norm(); }
    auto MahalanobisDistanceTo(double mux,
                               double muy,
                               double sdx,
                               double sdy,
        ...
        return std::sqrt((dx * dx * sdy * sdy - 2 * dx * dy * covariance +
                          dy * dy * sdx * sdx) /
                         (sdx * sdx * sdy * sdy - covariance * covariance));
    }

    T DistanceSquareTo(const Vec2<T> &vec) const {
    ...
    T Angle() const { return pnc_x::fast_math::Atan2(this->y(), this->x()); }

    T FastAngle() const { return fast_math::Atan2(this->y(), this->x()); }

    void SelfRotate(T yaw) {
        double tmp_x = this->x();
        this->x() = (this->x() * cos(yaw)) - (this->y() * sin(yaw));
        this->y() = (tmp_x * sin(yaw)) + (this->y() * cos(yaw));
    }
    ...
    Vec2<T> rotate(const Vec2<T> &unit) const {
    ...
    Vec2<T> Unit() const {
        ...
        XCHECK_NE(local_norm, 0.0);
        return {this->x() / local_norm, this->y() / local_norm};
    }
    T Length() const {
...
T NormalizeAngle2D(const Vec2<T> v1, const Vec2<T> v2) {
    ...
    return NormalizeAngle(pnc_x::fast_math::Atan2(det, dot));
}

template <typename T>
T NormalizeAngle2D(const Vec2<T> v) {
    return NormalizeAngle(pnc_x::fast_math::Atan2(v.y(), v.x()));
}
...
class Vec3 : public Eigen::Matrix<T, 3, 1> {
 public:
    ...
    Vec3() : Base(0, 0, 0) {}
    Vec3(T local_x, T local_y, T local_z) : Base(local_x, local_y, local_z) {}
    Vec3(Vec2<T> v, T local_z) : Base(v.x(), v.y(), local_z) {}

    explicit Vec3(const Vec3dProto &proto)
        : Base(proto.x(), proto.y(), proto.z()) {}

    template <typename OtherDerived>
    Vec3(const Eigen::EigenBase<OtherDerived> &vec) : Base(vec) {}
    ...
    void ToProto(U *proto) const {
        static_assert(std::is_base_of<google::protobuf::Message, U>::value,
                      "U must be derived from google::protobuf::Message");
        proto->set_x(this->x());
        proto->set_y(this->y());
        proto->set_z(this->z());
    }
