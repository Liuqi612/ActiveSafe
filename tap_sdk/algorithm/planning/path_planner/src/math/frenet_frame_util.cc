...
#include "path_planner/src/math/frenet_frame_util.h"
...
Vec2d SLToXY(const FrenetCoordinate &sl,
    ...
    XCHECK_EQ(anchor_points.size(), anchor_s.size());
    XCHECK_GT(anchor_s.size(), 1);
...
FrenetCoordinate XYToSLBruteForce(const Vec2d &xy,
    ...
    for (int i = 1; i < anchor_points.size(); ++i) {
        ...
        if (projection < 0.0 && i > 1) {
            ...
            s = anchor_s[static_cast<size_t>(i) - 1U];
        } else if ((projection > 1.0) && (i + 1 < static_cast<int>(anchor_points.size()))) {
            l = std::copysign(p1.DistanceTo(xy), production);
            s = anchor_s[i];
        } else {
            l = production;
            s = Lerp(anchor_s[static_cast<size_t>(i) - 1U], anchor_s[static_cast<size_t>(i)], projection);
        }
