...
double lerp_angle(const double a0,
                  const double t0,
                  const double a1,
                  const double t1,
                  const double t) {
...
Vec2d InterpolateUsingLinearApproximation(const Vec2d &p0,
                                          const Vec2d &p1,
                                          const double w) {
    Vec2d p;
    p.set_x(((1.0 - w) * p0.x()) + (w * p1.x()));
    p.set_y(((1.0 - w) * p0.y()) + (w * p1.y()));
    return p;
}
...
SLPoint InterpolateUsingLinearApproximation(const SLPoint &p0,
                                            ...
                                            const double w) {
    SLPoint p;
    p.s = ((1.0 - w) * p0.s) + (w * p1.s);
    p.l = ((1.0 - w) * p0.l) + (w * p1.l);
    return p;
}
