...
#include "path_planner/src/math/curve_limits.h"
...
void CurveLimit::Clear() { boundary_map_.clear(); };
bool CurveLimit::IsValid() const { return !boundary_map_.empty(); }
const std::map<double, std::pair<double, double>> &CurveLimit::GetBoundary()
    const {
...
bool CurveLimit::Create(const LineCurve1d &lower_boundary,
    ...
    if (!lower_boundary.IsValid() && !upper_boundary.IsValid()) {
        // std::cout << "No valid boundary, create CurveLimit fail!" <<
        // std::endl;
        return false;
    }
    ...
    while ((s < (s_end + Constants::ZERO)) && loop_guard) {
...
void CurveLimit::SetBoundary(const double &x,
                             const double &lower,
                             const double &upper) {
    if (lower < upper) {
...
bool CurveLimit::GetBoundary(const double &x,
                             double *lower,
                             double *upper) const {
...
bool CurveLimit::GetBoundary(
    std::vector<ads_x::planning::Point2d> &lower,
...
bool CurveLimit::GetBoundary(std::vector<double> &x,
