...
#include "path_planner/src/math/geometry/polygon2d_util.h"
...
double FindHalfPlaneIntersection(const HalfPlane &hp,
...
PolygonHalfPlaneOverlap ComputeConvexPolygonHalfPlaneOverlap(
    ...
    const double left_depth = hp.lat_proj(points[left]);
    if (left_depth > 0.0) {
        o.in = FindHalfPlaneIntersection(hp, polygon, left, lowest);
    } else {
        o.in = hp.lon_proj(points[left]);
    }
    const double right_depth = hp.lat_proj(points[right]);
    if (right_depth > 0.0) {
...
PolygonHalfPlaneOverlap ComputeNonPolygonHalfPlaneOverlap(
    ...
    PolygonHalfPlaneOverlap o;
    // XCHECK(false) << "Not implemented yet.";
    return o;
}
...
PolygonBoxOverlap ComputePolygonBoxOverlap(const Polygon2d &polygon,
    ...
    const auto left_hp_overlap =
        ComputePolygonHalfPlaneOverlap(polygon, left_hp);
    if (left_hp_overlap.dist > 0.0) {
        overlap.lat_dist = left_hp_overlap.dist;
        overlap.in = overlap.out =
            boost::algorithm::clamp(left_hp_overlap.in, 0.0, box_length);
        return overlap;
    }
    if (left_hp_overlap.out < 0.0) {
        overlap.in = overlap.out = 0.0;
        overlap.lat_dist = std::min(
            0.0, box.width() + ComputePolygonHalfPlaneLeftSideLateralDist(
                                   polygon, left_hp));
        return overlap;
    ...
    if (right_hp_overlap.dist > 0.0) {
        ...
        overlap.in = overlap.out = boost::algorithm::clamp(
            box.length() - right_hp_overlap.in, 0.0, box_length);
        return overlap;
    }

    overlap.lat_dist = 0.0;
    overlap.in =
        std::max(left_hp_overlap.in, box_length - right_hp_overlap.out);
    overlap.out =
        std::min(left_hp_overlap.out, box_length - right_hp_overlap.in);
    ...
    overlap.in = boost::algorithm::clamp(overlap.in, 0.0, box_length);
    overlap.out = boost::algorithm::clamp(overlap.out, 0.0, box_length);
    return overlap;
}
