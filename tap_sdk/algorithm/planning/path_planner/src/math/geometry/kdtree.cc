...
#include "path_planner/src/math/geometry/kdtree.h"
#include "path_planner/src/math/point_types.h"
...
KDTree<PointType>::KDTree(const std::vector<PointType> &points) {
    ...
    root_index_ = MakeTree(0, static_cast<int>(nodes_.size()), 0);
}
...
std::set<int> KDTree<PointType>::FindKNearest(const PointType &point,
                                              int k) const {
    XCHECK_LE(k, nodes_.size());
    std::set<int> nearest_indices;
    for (int i = 0; i < k; ++i) {
        int nearest_index;
        ScalarType best_dist_sqr = std::numeric_limits<ScalarType>::max();
        FindNearest<true>(point, root_index_, 0, nearest_indices,
                          &nearest_index, &best_dist_sqr);
...
std::set<int> KDTree<PointType>::FindNearestInRadius(
    ...
    while ((max_dist_sqr > 0) && loop_guard) {
        int nearest_index;
        ScalarType best_dist_sqr = std::numeric_limits<ScalarType>::max();
        FindNearest<true>(point, root_index_, 0, nearest_indices,
                          &nearest_index, &best_dist_sqr);
...
void KDTree<PointType>::FindNearest(const PointType &point,
                                    int node_index,
                                    int dim,
                                    const std::set<int> &to_ignore,
                                    int *best,
    ...
    auto loop_guard = DEFAULT_LOOP_GUARD_WITH_FUNC();
    while (!stack.empty() && loop_guard) {
        auto state = stack.back();
        stack.pop_back();
        int index = state.node_index;
        int this_dim = state.dim;
        ScalarType min_dist2 = state.min_dist2;
