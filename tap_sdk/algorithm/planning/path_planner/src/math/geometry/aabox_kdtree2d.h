...
class alignas(64) AABoxKDTree2dNode {
 public:
    ...
    using PriorityQueue =
        std::priority_queue<ObjectPtrWithDistanceSqr,
                            std::vector<ObjectPtrWithDistanceSqr>,
                            Compare>;
    ...
    AABoxKDTree2dNode(std::vector<ObjectPtr> objects,
        ...
        if (depth <= kMaxDepth && SplitToSubNodes(objects, params)) {
    ...
    std::vector<ObjectPtr> GetKNearestObjects(int want_k,
        ...
        PriorityQueue priority_queue =
            GetKNearestObjectsInternal(want_k, point);
        ...
        while (!priority_queue.empty() && loop_guard) {
            nearest_objects.push_back(priority_queue.top().first);
            priority_queue.pop();
        }
 ...
 private:
     ...
     public:
        ...
        explicit ObjectsInfo(int num_objects) : num_objects_(num_objects) {
            const int size_in_bytes =
                static_cast<int>((sizeof(ObjectPtr) * num_objects * 2) +
                                 (sizeof(double) * num_objects * 2));
            data_ = new char[static_cast<unsigned long>(size_in_bytes)];
        }
        ~ObjectsInfo() { delete[] data_; }

        ObjectPtr *objects_sorted_by_min() {
            return reinterpret_cast<ObjectPtr *>(data_);
        }
        ObjectPtr *objects_sorted_by_max() {
            return reinterpret_cast<ObjectPtr *>(data_) + num_objects_;
        }
        double *objects_sorted_by_min_bound() {
            return reinterpret_cast<double *>(
                reinterpret_cast<ObjectPtr *>(data_) + (num_objects_ * 2));
        }
        double *objects_sorted_by_max_bound() {
            return reinterpret_cast<double *>(
                       reinterpret_cast<ObjectPtr *>(data_) +
                       (num_objects_ * 2)) +
                   num_objects_;
        }
    ...
    void InitObjects(const std::vector<ObjectPtr> &objects) {
        ...
        if (partition_ == PARTITION_X) {
            std::stable_sort(objects_sorted_by_min,
                             objects_sorted_by_min + num_objects_,
                             [](ObjectPtr obj1, ObjectPtr obj2) {
                                 return obj1->ComputeAABox().min_x() <
                                        obj2->ComputeAABox().min_x();
                             });
            for (int i = 0; i < num_objects_; ++i) {
            ...
            std::stable_sort(objects_sorted_by_max,
                             objects_sorted_by_max + num_objects_,
                             [](ObjectPtr obj1, ObjectPtr obj2) {
                                 return obj1->ComputeAABox().max_x() >
                                        obj2->ComputeAABox().max_x();
                             });
            for (int i = 0; i < num_objects_; ++i) {
            ...
            std::stable_sort(objects_sorted_by_min,
                             objects_sorted_by_min + num_objects_,
                             [](ObjectPtr obj1, ObjectPtr obj2) {
                                 return obj1->ComputeAABox().min_y() <
                                        obj2->ComputeAABox().min_y();
                             });
            for (int i = 0; i < num_objects_; ++i) {
                objects_sorted_by_min_bound[i] =
            ...
            std::stable_sort(objects_sorted_by_max,
                             objects_sorted_by_max + num_objects_,
                             [](ObjectPtr obj1, ObjectPtr obj2) {
                                 return obj1->ComputeAABox().max_y() >
                                        obj2->ComputeAABox().max_y();
                             });
            for (int i = 0; i < num_objects_; ++i) {
                objects_sorted_by_max_bound[i] =
    ...
    bool SplitToSubNodes(const std::vector<ObjectPtr> &objects,
        ...
        if ((params.max_depth >= 0) &&
            (static_cast<int>(depth_) >= params.max_depth)) {
            return false;
        }
    ...
    void GetObjectsInternal(
        ...
        for (const auto &node : subnodes_) {
            if (node != nullptr) {
                node->GetObjectsInternal(point, distance, distance_sqr,
                                         result_objects);
            }
        }
    }
    ...
    void GetObjectsInternalWithinAABox(
        ...
        std::vector<ObjectPtr> *const result_objects) const {
        if (!XCHECK_NOTNULL(result_objects)) {
            return;
        }
        ...
        if (partition_ == PARTITION_X) {
            for (int i = 0; i < num_objects_; ++i) {
                if (objects_info_->objects_sorted_by_max_bound()[i] <
                    aabox.min_x()) {
                    break;
                }
                ObjectPtr object = objects_info_->objects_sorted_by_max()[i];
        ...
        for (const auto &node : subnodes_) {
            if (node != nullptr) {
                node->GetObjectsInternalWithinAABox(
                    aabox, is_aabox_contain_object, result_objects);
            }
        }
    }

    template <bool kSearchLeft>
    void GetNearestObjectFromSelfObjects(
        const Vec2d &point,
        ...
        for (int i = 0; i < num_objects_; ++i) {
            ...
            const bool bound_past = kSearchLeft ? (bound > pvalue) : (bound < pvalue);
            if (bound_past &&
    ...
    double GetKNearestObjectsFromSelfObjects(
        ...
        double min_dist2_upper_bound =
            priority_queue->empty() ? std::numeric_limits<double>::infinity()
    ...
    void GetNearestObjectInternal(const Vec2d &point,
        ...
        auto loop_guard = DEFAULT_LOOP_GUARD_WITH_FUNC();
        while (!node_stack.empty() && loop_guard) {
            auto node_pair = node_stack.back();
            node_stack.pop_back();

            const AABoxKDTree2dNode *node = node_pair.first;
            bool search_subtree = node_pair.second;
            ...
            const bool subnode_index =
                point[node->partition_] < node->ComputePartitionPosition() ? 0
                                                                           : 1;
            if (search_subtree) {
                node_stack.emplace_back(
                    node->subnodes_[1 - subnode_index].get(), true);
                node_stack.emplace_back(node, false);
                node_stack.emplace_back(node->subnodes_[subnode_index].get(),
                                        true);
            } else {
                if (subnode_index == 0) {
    ...
    PriorityQueue GetKNearestObjectsInternal(const int want_k,
        ...
        while (!node_stack.empty() && loop_guard) {
            ...
            if (node == nullptr ||
                (search_subtree && node->LowerDistanceSquareToPoint(point) >=
            ...
            if (search_subtree) {
                for (const auto &subnode : node->subnodes_) {
                    node_stack.emplace_back(subnode.get(), true);
                }
                node_stack.emplace_back(node, false);
            } else {
                const bool is_left =
                    point[node->partition_] < node->ComputePartitionPosition()
                        ? true
                        : false;
    ...
    void PartitionObjects(std::vector<ObjectPtr> objects,
        ...
        if (!XCHECK(XCHECK_NOTNULL(left_subnode_objects)->empty()) ||
            !XCHECK(XCHECK_NOTNULL(right_subnode_objects)->empty())) {
            return;
        }
 ...
 private:
    int num_objects_ = 0;
    uint16_t depth_ = static_cast<uint16_t>(0);
    ...
    std::unique_ptr<ObjectsInfo> objects_info_{nullptr};
