...
class Polygon2d {
 public:
    ...
    void SetFromBox(const Box2d &box);

    const std::vector<Vec2d> &points() const { return points_; }

    const std::vector<Segment2d> &line_segments() const {
    ...
    Vec2d centroid() const {
        return std::accumulate(points_.begin(), points_.end(),
                               Vec2d(0.0, 0.0)) /
               points_.size();
    }
    ...
    static boost::optional<std::vector<Vec2d>> ComputeConvexHullPoints(
    ...
    static bool AreNonDegeneratedConvexHullPoints(
        absl::Span<const Vec2d> points);
    ...
    bool GetOverlap(const Segment2d &line_segment,
                    Vec2d *const first,
    ...
    Box2d NarrowAreaSegmentBox(const Vec2d &p1, const Vec2d &p2) const;
    ...
    void ExtremePoints(const Vec2d &direction_vec,
                       Vec2d *first,
                       Vec2d *last) const;
    ...
    [[nodiscard]] Polygon2d Transform(const Vec2d &center,
                                      double cos_angle,
                                      double sin_angle,
                                      const Vec2d &translation) const;

    [[nodiscard]] Polygon2d AffineTransform(const Vec2d &center,
                                            double cos_angle,
                                            double sin_angle,
                                            const Vec2d &translation) const;
    ...
    [[nodiscard]] boost::optional<Polygon2d> MinkowskiSumWithVec(
        const Vec2d &vec) const;

    std::string DebugString() const;

    std::string DebugStringFullPrecision() const {
        return absl::StrCat(pnc_x::DebugStringFullPrecision(points_),
                            ", /*is_convex=*/", is_convex());
    }

    template <typename Container>
    static boost::optional<Polygon2d> FromPoints(const Container &local_points,
                                                 bool is_convex) {
        if (local_points.size() < 3) { return boost::none; }
        std::vector<Vec2d> pts;
        pts.reserve(local_points.size());
