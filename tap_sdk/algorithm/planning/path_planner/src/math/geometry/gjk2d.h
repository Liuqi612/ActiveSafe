...
class Gjk2d {
 ...
 private:
    ...
    static Vec2d UpdateSearchDirectionFromSegment(std::array<Vec2d, 3> *simplex,
                                                  int *cardinality);
    static Vec2d ConeRegion(std::array<Vec2d, 3> *simplex,
                            int index,
                            int *cardinality);
    static Vec2d UpdateSearchDirectionFromTriangle(
        std::array<Vec2d, 3> *simplex, int *cardinality);
    static Vec2d SubDistance(std::array<Vec2d, 3> *simplex, int *cardinality);
    template <typename F>
    static boost::optional<std::array<Vec2d, 3>> GetGjkSimplex(
        const Vec2d &point_inside_hint,
        const F &calculate_extreme_point,
        double min_distance_sqr,
        int max_iterations);
...
inline Vec2d Gjk2d::ConeRegion(std::array<Vec2d, 3> *simplex,
                               int index,
                               int *cardinality) {
    std::swap((*simplex)[static_cast<size_t>(index)],
              (*simplex)[static_cast<size_t>(2)]);
    const auto &m = (*simplex)[0];
    const auto &n = (*simplex)[1];
    const auto &v = (*simplex)[static_cast<size_t>(2)];
    const auto mv = v - m;
    const auto nv = v - n;
    if (mv.dot(nv) < 0.0) {
        if (v.dot(mv) > 0.0) {
...
inline Vec2d Gjk2d::UpdateSearchDirectionFromTriangle(
    ...
    const auto sigma_u =
        (*simplex)[1U].CrossProd((*simplex)[static_cast<size_t>(2)]);
    const auto sigma_v =
        (*simplex)[static_cast<size_t>(2)].CrossProd((*simplex)[0]);
    ...
    switch (barycode) {
        ...
        case 3:
            ...
            (*simplex)[1U] = std::move((*simplex)[static_cast<size_t>(2)]);
            return UpdateSearchDirectionFromSegment(simplex, cardinality);
        case 4:
            return ConeRegion(simplex, 0, cardinality);
        case 5:
            (*simplex)[1U] = std::move((*simplex)[2]);
            return UpdateSearchDirectionFromSegment(simplex, cardinality);
        case 6:
            return UpdateSearchDirectionFromSegment(simplex, cardinality);
        case 7:
...
template <typename F>
inline boost::optional<std::array<Vec2d, 3>> Gjk2d::GetGjkSimplex(
    const Vec2d &point_inside_hint,
    const F &calculate_extreme_point,
    double min_distance_sqr,
    int max_iterations) {
...
inline double Gjk2d::DistanceSquareTo(const Vec2d &point_inside_hint,
    ...
    while ((cardinality < 3) && (++iter <= max_iterations)) {
        const auto w = -calculate_extreme_point(support_vec);
        if (support_vec.dot(w) >=
            ((1.0 - min_distance_sqr) * support_vec.Sqr())) {
            break;
        }
        simplex[cardinality] = -w;
        ++cardinality;
...
inline bool Gjk2d::GetMinPenetrationDistance(Vec2d point_inside_hint,
                                             ...
                                             double *min_penetration,
                                             Vec2d *dir_vec) {
    if (!XCHECK_NOTNULL(min_penetration) || !XCHECK_NOTNULL(dir_vec)) {
        return false;
    }
...
inline bool Gjk2d::GetPenetrationDistanceAlongDir(
    Vec2d point_inside_hint,
    const F &calculate_extreme_point,
    const Vec2d &dir_vec,
    double min_distance,
    int max_iterations,
    double *penetration) {
    if (!XCHECK_NOTNULL(penetration)) {
        return false;
    }
    const double min_distance_sqr = Sqr(min_distance);
