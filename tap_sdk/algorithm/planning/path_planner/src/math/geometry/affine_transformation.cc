...
#include "path_planner/src/math/geometry/affine_transformation.h"
...
Vec3d SkewSymmetricMatrixToVector(const Mat3d &u_cross) {
    XCHECK_LT(std::abs(u_cross(2, 1) + u_cross(1, 2)), kEpsilon);
    XCHECK_LT(std::abs(u_cross(0, 2) + u_cross(2, 0)), kEpsilon);
    XCHECK_LT(std::abs(u_cross(1, 0) + u_cross(0, 1)), kEpsilon);
    XCHECK_LT(std::abs(u_cross(0, 0)), kEpsilon);
    XCHECK_LT(std::abs(u_cross(1, 1)), kEpsilon);
    XCHECK_LT(std::abs(u_cross(2, 2)), kEpsilon);
    return Vec3d(u_cross(2, 1), u_cross(0, 2), u_cross(1, 0));
}

Mat3d RotationMatrixFromAngleAndUnitAxis(double cos_theta,
                                         double sin_theta,
                                         const Vec3d &u) {
...
void AngleAndUnitAxisFromRotationMatrix(const Mat3d &rot,
                                        double *theta,
                                        Vec3d *u) {
    if (!XCHECK(theta != nullptr) || !XCHECK(u != nullptr)) {
        return;
    }
    ...
    XCHECK_LT(sin_theta, 1.0 + kEpsilon);
    sin_theta = std::min(1.0, sin_theta);

    double cos_theta = ((rot.trace() - 1.0) * 0.5);
    XCHECK_LT(cos_theta, 1.0 + kEpsilon);
    XCHECK_GT(cos_theta, -1.0 - kEpsilon);
    cos_theta = std::min(1.0, std::max(-1.0, cos_theta));

    if (n < kEpsilon) {
        if (rot(0, 0) < (-1.0 + kEpsilon) &&
            rot(1, 1) < (-1.0 + kEpsilon)) {
            *theta = std::acos(cos_theta);
            *u = Vec3d(0.0, 0.0, 1.0);
            return;
        } else if ((rot(0, 0) < (-1.0 + kEpsilon) &&
                    rot(2, 2) < (-1.0 + kEpsilon))) {
            *theta = std::acos(cos_theta);
            *u = Vec3d(0.0, 1.0, 0.0);
            return;
        } else if ((rot(1, 1) < (-1.0 + kEpsilon) &&
                    rot(2, 2) < (-1.0 + kEpsilon))) {
            *theta = std::acos(cos_theta);
            *u = Vec3d(1.0, 0.0, 0.0);
            return;
...
AffineTransformation AffineTransformation::FromExtrinsics(
    const ExtrinsicsType &extrinsics) {
...
AffineTransformation AffineTransformation::operator*(const Mat4d &mat) const {
...
const Mat4d &AffineTransformation::mat() const { return mat_; }

Vec4d AffineTransformation::Transform(const Vec4d &x) const { return mat_ * x; }

Vec3d AffineTransformation::TransformPoint(const Vec3d &x) const {
    return Vec3d(mat_.block<3, 3>(0, 0) * x + mat_.block<3, 1>(0, 3));
}

Vec3d AffineTransformation::TransformVector(const Vec3d &x) const {
    return Vec3d(mat_.block<3, 3>(0, 0) * x);
}

Vec3d AffineTransformation::TransformCovector(const Vec3d &x) const {
...
AffineTransformation::AffineTransformation(const std::vector<double> &coeffs) {
    SetIdentity();
    XCHECK(coeffs.size() == 9 || coeffs.size() == 16);
    if (coeffs.size() == 9) {
...
AffineTransformation AffineTransformation::FromTranslation(double x,
                                                           double y,
                                                           double z) {
...
AffineTransformation AffineTransformation::FromScaling(double sx,
                                                       double sy,
                                                       double sz) {
...
AffineTransformation &AffineTransformation::SetTranslation(double x,
                                                           double y,
                                                           double z) {
    mat_ = Mat4d::Identity();
    mat_(0, 3) = x;
...
AffineTransformation &AffineTransformation::SetRotation(double theta,
    ...
    mat_.block<3, 3>(0, 0) = RotationMatrixFromAngleAndUnitAxis(
        cos_theta, sin_theta, normalized_axis);
    return *this;
}
...
AffineTransformation &AffineTransformation::SetRotation(Quaternion quaternion) {
    ...
    XCHECK_LT(std::abs(cos_theta_2_sqr + sin_theta_2_sqr - 1.0),
              kLargerEpsilon);

    if (sin_theta_2 < kEpsilon) {
        return *this;
    }
    const Vec3d axis = Vec3d(quaternion.vec()) / sin_theta_2;
...
AffineTransformation &AffineTransformation::SetScaling(double sx,
                                                       double sy,
                                                       double sz) {
    mat_ = Mat4d::Identity();
    mat_(0, 0) = sx;
...
AffineTransformation &AffineTransformation::Set(const Mat4d &mat) {
    XCHECK_EQ(mat(3, 0), 0.0);
    XCHECK_EQ(mat(3, 1), 0.0);
    XCHECK_EQ(mat(3, 2), 0.0);
    XCHECK_EQ(mat(3, 3), 1.0);
    mat_ = mat;
    return *this;
}
    ...
    for (const AffineTransformationSequenceProto::Operation &operation :
        ...
        switch (operation.operation_case()) {
            ...
            case AffineTransformationSequenceProto::Operation::kScaling:
                ...
                break;
            case AffineTransformationSequenceProto::Operation::
                OPERATION_NOT_SET:
            default:
                break;
        }
...
AffineTransformation &AffineTransformation::ApplyTranslation(double x,
                                                             double y,
                                                             double z) {
...
AffineTransformation &AffineTransformation::ApplyScaling(double sx,
                                                         double sy,
                                                         double sz) {
    mat_.col(0) *= sx;
    mat_.col(1) *= sy;
...
void AffineTransformation::GetRotation(double *theta, Vec3d *axis) const {
    if (!XCHECK(theta != nullptr) || !XCHECK(axis != nullptr)) {
        return;
    }
    Mat3d u, v;
    Vec3d s;
    ...
    XCHECK_LT(std::abs(s.x() - s.y()), kEpsilon);
    XCHECK_LT(std::abs(s.x() - s.z()), kEpsilon);
