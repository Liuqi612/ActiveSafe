...
#include "path_planner/src/math/geometry/intersection_util.h"
...
bool FindIntersectionBetweenLinesWithTangents(const Vec2d &p0,
                                              ...
                                              double *s1,
                                              Vec2d *inter_point) {
    if (!XCHECK(s0 != nullptr) || !XCHECK(s1 != nullptr) ||
        !XCHECK(inter_point != nullptr)) {
        return false;
    }
...
bool FindFirstIntersectionBetweenCurves(const Polyline2d &curve1,
                                        ...
                                        double *arc_len1,
                                        double *arc_len2) {
    if (!XCHECK_NOTNULL(inter_point)) {
        return false;
    }

    for (size_t i = 0; i + 1 < curve1.points().size(); ++i) {
        const Segment2d seg1(curve1.points()[i], curve1.points()[i + 1]);
        for (size_t j = 0; j + 1 < curve2.points().size(); ++j) {
            const Segment2d seg2(curve2.points()[j], curve2.points()[j + 1]);
            if (seg1.GetIntersect(seg2, inter_point)) {
                *arc_len1 = curve1.point_s()[i] +
                            inter_point->DistanceTo(curve1.points()[i]);

                *arc_len2 = curve2.point_s()[j] +
                            inter_point->DistanceTo(curve2.points()[j]);
...
FindIntersectionBetweenCircle2dSegment2d(const Circle2d &circle,
    ...
    std::pair<boost::optional<Vec2d>, boost::optional<Vec2d>> down_up;
    ...
    if ((up_intersection_proj >= 0.0) &&
        (up_intersection_proj <= segment.length())) {
        down_up.second =
...
void UpdatePairOptional(
    const std::pair<boost::optional<Vec2d>, boost::optional<Vec2d>> &input,
    std::pair<boost::optional<Vec2d>, boost::optional<Vec2d>> *buffer) {
    if (!XCHECK_NOTNULL(buffer)) {
        return;
    }
...
FindIntersectionBetweenArc2dHRSegment2d(const Arc2dHR &arc_hr,
    ...
    if (arc_hr.is_inferior_arc()) {
        ...
        if (ClampSegment2dByHalfPlane(arc_hr.start_bound(), &seg_part_1)) {
            UpdatePairOptional(FindIntersectionBetweenCircle2dSegment2d(
                                   arc_hr.circle(), seg_part_1),
                               &down_up);
        }
...
bool InferiorArcHasLeftIntersectionWithCircle(
    double s_to_other_circle_center_sqr,
    double e_to_other_circle_center_sqr,
    double other_circle_radius_sqr,
    bool s_on_left,
    bool e_on_left) {
...
bool InferiorArcHasRightIntersectionWithCircle(
    ...
    bool e_on_right) {
    if (s_on_right &&
        (s_to_other_circle_center_sqr < other_circle_radius_sqr)) {
        return false;
    }
    if (e_on_right &&
        (e_to_other_circle_center_sqr > other_circle_radius_sqr)) {
        return false;
    }
...
FindIntersectionBetweenArc2dHRs(const Arc2dHR &a1, const Arc2dHR &a2) {
...

    {
        ...
        if (a1.is_inferior_arc()) {
            has_left_intersection = has_left_intersection &&
                                    InferiorArcHasLeftIntersectionWithCircle(
                                        s1_to_c2_sqr, e1_to_c2_sqr, r2_sqr,
                                        s1_prod >= 0.0, e1_prod >= 0.0);
            has_right_intersection = has_right_intersection &&
                                     InferiorArcHasRightIntersectionWithCircle(
                                         s1_to_c2_sqr, e1_to_c2_sqr, r2_sqr,
                                         s1_prod <= 0.0, e1_prod <= 0.0);
        } else {
            has_left_intersection = has_left_intersection &&
    ...
    std::pair<boost::optional<Vec2d>, boost::optional<Vec2d>> left_right;
