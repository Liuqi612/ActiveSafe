...
#include "path_planner/src/math/geometry/gjk2d.h"
...
inline double CrossProd(const Vec2d &start_point,
...
Polygon2d::Polygon2d(std::vector<Vec2d> points, bool is_convex)
    ...
    if (is_convex_) {
        XCHECK(AreConvexHullPoints(points_));
    }
}
...
void Polygon2d::SetFromBox(const Box2d &box) {
    points_.clear();
    points_.reserve(static_cast<size_t>(4));
...
bool Polygon2d::IsSelfIntersecting() const {
    ...
    for (int i = 0; i < n; ++i) {
        const auto &seg_i = line_segments_[static_cast<size_t>(i)];
        for (int j = i + 2; j < (n - 1); ++j) {
            if (seg_i.HasIntersect(line_segments_[static_cast<size_t>(j)])) {
                return true;
            }
        }
    }
...
double Polygon2d::DistanceTo(const Vec2d &point) const {
    XCHECK_GE(points_.size(), 3);
    if (IsPointIn(point)) {
        return 0.0;
    }
    double distance = std::numeric_limits<double>::infinity();
    for (int i = 0; i < num_points_; ++i) {
        distance = std::min(distance, line_segments_[static_cast<size_t>(i)].DistanceTo(point));
    }
    return distance;
}
...
double Polygon2d::DistanceSquareTo(const Vec2d &point) const {
    ...
    for (int i = 0; i < num_points_; ++i) {
        distance_sqr =
            std::min(distance_sqr, line_segments_[static_cast<size_t>(i)].DistanceSquareTo(point));
    }
    return distance_sqr;
}
...
double Polygon2d::DistanceTo(const Segment2d &line_segment) const {
    ...
    for (int i = 0; i < num_points_; ++i) {
        distance = std::min(distance, line_segment.DistanceTo(points_[static_cast<size_t>(i)]));
    }
    return distance;
}
...
double Polygon2d::DistanceTo(const Box2d &box) const {
    ...
    double distance = std::numeric_limits<double>::infinity();
    for (int i = 0; i < num_points_; ++i) {
        distance = std::min(distance, box.DistanceTo(line_segments_[static_cast<size_t>(i)]));
    }
    return distance;
}
...
double Polygon2d::ConvexDistanceToConvex(const Polygon2d &polygon) const {
    ...
    constexpr double kMinDistThresholdSqr = 1e-4 * 1e-4;
    const int max_iterations = num_points() + polygon.num_points();

    return Gjk2d::DistanceTo(random_point_inside_c,
                             calculate_extreme_point_of_c, kMinDistThresholdSqr,
                             max_iterations);
}

double Polygon2d::DistanceTo(const Polygon2d &polygon) const {
    XCHECK_GE(points_.size(), 3);
    XCHECK_GE(polygon.num_points(), 3);
    ...
    double distance = std::numeric_limits<double>::infinity();
    for (int i = 0; i < num_points_; ++i) {
        distance = std::min(distance, polygon.DistanceTo(line_segments_[static_cast<size_t>(i)]));
    }
    return distance;
}
...
bool Polygon2d::HasOverlapWithBuffer(const Box2d &box,
                                     double lat_buffer,
    ...
    if ((min_x > this->max_x()) || (max_x < this->min_x()) ||
        (min_y > this->max_y()) || (max_y < this->min_y())) {
        return false;
    }
...
bool Polygon2d::ConvexHasOverlapConvex(const Polygon2d &polygon) const {
    ...
    return Gjk2d::HasOverlap(random_point_inside_c,
                             calculate_extreme_point_of_c, kMinDistThresholdSqr,
                             max_iterations);
}
...
bool Polygon2d::GetMinPenetrationDistance(const Polygon2d &polygon,
    ...
    constexpr double kMinDistThreshold = 1e-4;
    const int max_iterations = num_points() + polygon.num_points();
...
void Polygon2d::BuildFromPoints() {
    ...
    // XCHECK_GE(num_points_, 3);

    area_ = 0.0;
    for (int i = 1; i < num_points_; ++i) {
        const int im1 = i - 1;
        area_ += CrossProd(points_[0], points_[static_cast<size_t>(im1)], points_[static_cast<size_t>(i)]);
    }
    if (area_ < 0.0) {
        area_ = -area_;
    ...
    line_segments_.reserve(static_cast<size_t>(num_points_));
    for (int i = 0; i < num_points_; ++i) {
        const int next_i = Next(i);
        line_segments_.emplace_back(points_[static_cast<size_t>(i)], points_[static_cast<size_t>(next_i)]);
    }
...
bool Polygon2d::AreConvexHullPoints(absl::Span<const Vec2d> points) {
    ...
    if (n < 3) return false;
    const auto next = [n](int x) {
        return (x + 1) >= n ? (x + 1 - n) : (x + 1);
    };
    const auto prev = [n](int x) { return x == 0 ? n - 1 : x - 1; };
    for (int i = 0; i < n; ++i) {
        const int prev_i = prev(i);
        const int next_i = next(i);
        const auto dd = CrossProd(points[static_cast<size_t>(prev_i)], points[static_cast<size_t>(i)], points[static_cast<size_t>(next_i)]);
        if (dd <= kMathEpsilon) {
            // AD_LINFO(PP) << absl::StrFormat(
...
bool Polygon2d::AreNonDegeneratedConvexHullPoints(
    ...
    XCHECK_GE(n, 3);
    Vec2d prev_seg = points[0] - points[static_cast<size_t>(n - 1)];
    for (int i = 0; i < n; ++i) {
        const int next_id = (i == (n - 1)) ? 0 : (i + 1);
        const Vec2d current_seg = points[static_cast<size_t>(next_id)] - points[static_cast<size_t>(i)];
...
boost::optional<std::vector<Vec2d>> Polygon2d::ComputeConvexHullPoints(
    absl::Span<const Vec2d> points) {
    const int origin_n = static_cast<int>(points.size());

    std::vector<int> sorted_by_x_indices(static_cast<size_t>(origin_n), 0);
    ...
    sorted_indices.reserve(static_cast<size_t>(origin_n));
    ...
    for (int i = 0; i < origin_n; ++i) {
        ...
        const Vec2d &point_i = points[static_cast<size_t>(sorted_by_x_indices[static_cast<size_t>(i)])];
        for (int j = i + 1; j < origin_n; ++j) {
            const Vec2d &point_j = points[static_cast<size_t>(sorted_by_x_indices[static_cast<size_t>(j)])];
        ...
        if (!has_duplicated_points)
            sorted_indices.push_back(sorted_by_x_indices[static_cast<size_t>(i)]);
    }
    if (sorted_indices.size() < 3UL) return boost::none;
    ...
    for (int i = 0; i < (n + n); ++i) {
        ...
        const int si_idx = (i < n) ? i : (n + n - 1 - i);
        const int idx = sorted_indices[static_cast<size_t>(si_idx)];
        const Vec2d &pt = points[static_cast<size_t>(idx)];
        auto loop_guard = DEFAULT_LOOP_GUARD();
        while ((count > last_count) &&
               (CrossProd(points[static_cast<size_t>(results[static_cast<size_t>(count - 2)])],
                          points[static_cast<size_t>(results[static_cast<size_t>(count - 1)])],
                         pt) <= kEpsilon) &&
               loop_guard) {
            results.pop_back();
            --count;
    ...
    convex_hull_points.reserve(static_cast<size_t>(count));
    for (const auto idx : results) {
        convex_hull_points.push_back(points[static_cast<size_t>(idx)]);
    }
...
bool Polygon2d::ComputeConvexHull(absl::Span<const Vec2d> points,
    ...
    if (!XCHECK_NOTNULL(polygon)) {
        return false;
    }
    auto result_points = ComputeConvexHullPoints(points);
    if (result_points.has_value()) {
        *polygon = Polygon2d(std::move(*result_points), true);
        return true;
    }
    return false;
}
...
Polygon2d Polygon2d::MergeTwoBoxes(const Box2d &first_box,
    ...
    XCHECK(Polygon2d::ComputeConvexHull(corners, &merged_polygon));
...
bool Polygon2d::ClipConvexHull(const Segment2d &line_segment,
    ...
    for (size_t i = 0UL; i < n; ++i) {
        ...
        const size_t j = (i == (n - 1UL)) ? 0UL : (i + 1UL);
        if (side[i] * side[j] < 0) {
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
    ...
    if (!XCHECK_GE(points_.size(), 3) || !XCHECK_NOTNULL(overlap_polygon) ||
        !XCHECK(is_convex_ && other_polygon.is_convex())) {
        return false;
    }

    std::vector<Vec2d> points = other_polygon.points();
    for (int i = 0; i < num_points_; ++i) {
        if (!ClipConvexHull(line_segments_[static_cast<size_t>(i)], &points)) {
            return false;
        }
    }
...
bool Polygon2d::HasOverlap(const Segment2d &line_segment) const {
    XCHECK_GE(points_.size(), 3);
    AABox2d aabox = SegmentToAABox(line_segment);
    if (!aabox.HasOverlap(aabox_)) {
        return false;
    }
...
bool Polygon2d::GetOverlap(const Segment2d &line_segment,
                           Vec2d *const first,
                           Vec2d *const last) const {
    if (!XCHECK_GE(points_.size(), 3) || !XCHECK_NOTNULL(first) ||
        !XCHECK_NOTNULL(last)) {
        return false;
    }
    ...
    for (const auto &poly_seg : line_segments_) {
        ...
        if (poly_seg.GetIntersect(line_segment, &pt)) {
            ...
            proj = boost::algorithm::clamp(proj, 0.0, line_segment.length());
            if (proj <= min_proj) {
                min_proj = proj;
                *first = pt;
            }
...
std::vector<Segment2d> Polygon2d::GetAllOverlaps(
    ...
    for (size_t i = 0; i + 1 < projections.size(); ++i) {
        ...
        if (overlaps.empty() ||
            (start_proj > overlaps.back().second + kEpsilon)) {
...
void Polygon2d::ExtremePoints(const Vec2d &direction_vec,
                              Vec2d *first,
                              Vec2d *last) const {
    if (!XCHECK(first != nullptr) || !XCHECK(last != nullptr)) {
        return;
    }
    int first_index;
    int last_index;
    ExtremePoints(direction_vec, &first_index, &last_index);
    *first = points()[static_cast<size_t>(first_index)];
    *last = points()[static_cast<size_t>(last_index)];
}
...
void Polygon2d::ExtremePoints(const Vec2d &direction_vec,
                              ...
                              Vec2d *first_pt,
                              Vec2d *last_pt) const {
    if (!XCHECK(first_idx != nullptr) || !XCHECK(last_idx != nullptr) ||
        !XCHECK(first_pt != nullptr) || !XCHECK(first_pt != nullptr)) {
        return;
    }
    ExtremePoints(direction_vec, first_idx, last_idx);
    *first_pt = points()[static_cast<size_t>(*first_idx)];
    *last_pt = points()[static_cast<size_t>(*last_idx)];
}
...
int Polygon2d::ConvexExtremePoint(const Vec2d &direction_vec) const {
    ...
    while ((++index < static_cast<int>(points_.size())) && loop_guard) {
        double product = direction_vec.dot(points_[static_cast<size_t>(index)]);
        if (product > max_proj) {
            max_proj = product;
            ret = index;
        } else {
    ...
    for (int i = static_cast<int>(points_.size()) - 1; i > index; --i) {
        double product = direction_vec.dot(points_[static_cast<size_t>(i)]);
        if (product > max_proj) {
            max_proj = product;
            ret = i;
        } else {
...
int Polygon2d::ExtremePointBruteForce(const Vec2d &direction_vec) const {
    ...
    for (int i = 0; i < num_points_; ++i) {
        const double proj = points_[static_cast<size_t>(i)].dot(direction_vec);
        if (proj > max_proj) {
            max_proj = proj;
            ret = i;
        }
...
int Polygon2d::ConvexExtremePointBinarySearch(
    ...
    auto before_extreme_point = [&](int raw_index) {
        ...
        if (start_at_right) {
            if (is_right_point(index)) {
                return (points_[index] - points_[start]).Dot(direction_vec) >=
                       -1e-10;
            } else {
                return false;
            }
    ...
    int l = start;
    int r = start + num_points_ - 1;
    XCHECK(before_extreme_point(l));
    XCHECK(!before_extreme_point(r));
    auto loop_guard = DEFAULT_LOOP_GUARD_WITH_FUNC();
    while (((l + 1) < r) && loop_guard) {
        int mid = (l + r) / 2;
...
void Polygon2d::ExtremePoints(const Vec2d &direction_vec,
                              int *first,
                              int *last) const {
    if (!XCHECK_GE(points_.size(), 3) || !XCHECK(first != nullptr) ||
        !XCHECK(last != nullptr)) {
        return;
    }
...
Box2d Polygon2d::NarrowAreaSegmentBox(const Vec2d &p1, const Vec2d &p2) const {
    if (p1.DistanceSquareTo(p2) < kEpsilon) {
        return Box2d(p1, 0.0, sqrt(kEpsilon), sqrt(kEpsilon));
    }
    Vec2d center(0.5 * (p1.x() + p2.x()), 0.5 * (p1.y() + p2.y()));
    Segment2d s(p1, p2);
    return Box2d(center, s.heading(), s.length(), sqrt(kEpsilon));
}
...
Box2d Polygon2d::MinAreaBoundingBox() const {
    ...
    if (!is_convex_) {
        ...
        if ((convex_polygon.num_points() == 0) ||
            (!convex_polygon.is_convex())) {
            for (int idx = 1; idx < num_points(); ++idx) {
                if (points_[0].DistanceSquareTo(points_[static_cast<size_t>(idx)]) > kEpsilon) {
                    return NarrowAreaSegmentBox(points_[0], points_[idx]);
                }
            }
            return NarrowAreaSegmentBox(points_[0], points_[1]);
        }
    ...
    int top_most = 0;
    for (int i = 0; i < num_points_; ++i) {
        const auto &line_segment = line_segments_[static_cast<size_t>(i)];
        double proj = 0.0;
        double min_proj = line_segment.ProjectOntoUnit(points_[static_cast<size_t>(left_most)]);
        auto loop_guard = DEFAULT_LOOP_GUARD();
        ...
        auto loop_guard1 = DEFAULT_LOOP_GUARD();
        ...
        double max_proj = line_segment.ProjectOntoUnit(points_[static_cast<size_t>(right_most)]);
        ...
        auto loop_guard3 = DEFAULT_LOOP_GUARD();
        while (((proj = line_segment.ProjectOntoUnit(
        ...
        double prod = 0.0;
        double max_prod = line_segment.ProductOntoUnit(points_[static_cast<size_t>(top_most)]);
        auto loop_guard5 = DEFAULT_LOOP_GUARD();
        ...
        auto loop_guard6 = DEFAULT_LOOP_GUARD();
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
    if (!is_convex_) {
        Polygon2d convex_polygon;
        XCHECK(ComputeConvexHull(points_, &convex_polygon));

        XCHECK(convex_polygon.is_convex());
    ...
    for (int i = 0; i < num_points_; ++i) {
        const int prev_i = Prev(i);
        const double start_angle = line_segments_[static_cast<size_t>(prev_i)].heading() - M_PI_2;
        const double end_angle = line_segments_[static_cast<size_t>(i)].heading() - M_PI_2;
        const double diff = NormalizeAngle(end_angle - start_angle);
        if (diff <= kEpsilon) {
            points.push_back(points_[static_cast<size_t>(i)] +
            ...
            for (int k = 0; k <= count; ++k) {
                ...
                points.push_back(points_[static_cast<size_t>(i)] +
                                 Vec2d::UnitFromAngle(angle) * distance);
            }
        }
    }
    Polygon2d new_polygon;
    XCHECK(ComputeConvexHull(points, &new_polygon));
...
Polygon2d Polygon2d::ExtrudeAlongVector(const Vec2d &vec) const {
    std::vector<Vec2d> points;
    points.reserve(static_cast<size_t>(num_points_) + 2U);
    const double vec_angle = vec.Angle();
    for (int i = 0; i < num_points_; ++i) {
        const int prev_i = Prev(i);
        const double start_angle =
            NormalizeAngle(line_segments_[static_cast<size_t>(prev_i)].heading() - vec_angle);
        const double end_angle =
            NormalizeAngle(line_segments_[static_cast<size_t>(i)].heading() - vec_angle);
        if ((start_angle < 0.0) && (end_angle >= 0.0)) {
            points.push_back(points_[static_cast<size_t>(i)]);
        } else if ((start_angle >= 0.0) && (end_angle < 0.0)) {
            points.push_back(points_[static_cast<size_t>(i)] + vec);
        }
        points.push_back(points_[static_cast<size_t>(i)] +
                         (end_angle > 0.0 ? vec : Vec2d::Zero()));
    }
    return Polygon2d(std::move(points));
}

Polygon2d Polygon2d::Transform(const Vec2d &center,
                               double cos_angle,
                               double sin_angle,
                               const Vec2d &translation) const {
    XCHECK_GE(Sqr(cos_angle) + Sqr(sin_angle), 1.0 - 1e-4);
    XCHECK_LE(Sqr(cos_angle) + Sqr(sin_angle), 1.0 + 1e-4);
    std::vector<Vec2d> points;
...
Polygon2d Polygon2d::AffineTransform(const Vec2d &center,
                                     double cos_angle,
                                     double sin_angle,
                                     const Vec2d &translation) const {
    XCHECK_GE(Sqr(cos_angle) + Sqr(sin_angle), 1.0 - 1e-4);
...
boost::optional<Polygon2d> Polygon2d::MinkowskiSumWithVec(
    ...
    std::vector<Vec2d> new_points;
    new_points.reserve(static_cast<size_t>(num_points_) + 2U);
    ...
    for (int index = left_most_index; !reached_right; index = Next(index)) {
        ...
        new_points.push_back(points_[static_cast<size_t>(index)]);
    }
    ...
    for (int index = right_most_index; !reached_left; index = Next(index)) {
        ...
        new_points.push_back(points_[static_cast<size_t>(index)] + vec);
    }

    if (new_points.size() >= 3) {
        return Polygon2d(std::move(new_points), true);
    } else {
        return {};
    }
}
...
std::string Polygon2d::DebugString() const {
    ...
    return absl::StrCat(
        "polygon2d (  num_points = ", num_points_, "  points = (", points_str,
...
bool Polygon2d::IsPointInSlow(const Vec2d &point) const {
    ...
    int c = 0;
    for (int i = 0; i < num_points_; ++i) {
        if ((points_[static_cast<size_t>(i)].y() > point.y()) != (points_[static_cast<size_t>(j)].y() > point.y())) {
            const double side =
                Vec2d(points_[static_cast<size_t>(i)] - point).CrossProd(points_[static_cast<size_t>(j)] - point);
