...
#include "path_planner/src/math/line_segment2d.h"
...
class LineCurve2d {
 public:
    ...
    bool SamplePoints(const double start_s,
                      const double length,
                      std::vector<Vec2d> *const points,
                      const double interval = 3.0) const;

    double GetDistance(const Vec2d &point,
                       Vec2d *nearest_point,
    ...
    bool GetProjection(const Vec2d &point,
                       double *accumulate_s,
