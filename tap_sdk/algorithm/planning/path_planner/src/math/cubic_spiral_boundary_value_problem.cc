...
#include "path_planner/src/math/cubic_spiral_boundary_value_problem.h"

#include <algorithm>
#include <cmath>

#include "path_planner/src/math/geometry/LBFGSpp/LBFGS.h"
...
namespace pnc_x {
namespace planning {
inline double CubicSpiralBoundaryValueProblem::Theta(
    double p0, double p1, double p2, double p3, double sg, double ratio) const {
    ...
    const double theta = (p0 * k * sg) - (0.11e2 / 0.4e1 * p0 * t4) +
                         (0.9e1 / 0.2e1 * p1 * t4) - (0.9e1 / 0.4e1 * p2 * t4) +
                         ((p3 * t4) / 0.2e1) + (0.3e1 * p0 * t14) -
                         (0.15e2 / 0.2e1 * p1 * t14) + (0.6e1 * p2 * t14) -
                         (0.3e1 / 0.2e1 * p3 * t14) - (0.9e1 / 0.8e1 * p0 * t24) +
                         (0.27e2 / 0.8e1 * p1 * t24) - (0.27e2 / 0.8e1 * p2 * t24) +
                         (0.9e1 / 0.8e1 * p3 * t24);
    return theta;
}

inline Vec3d CubicSpiralBoundaryValueProblem::ThetaGrad(
    double p0, double p1, double p2, double p3, double sg, double ratio) const {
    Vec3d grad;
    const double k = ratio;
    double t1 = k * k;
    ...
    grad[2] = (k * ((k * ((((-0.15e2 / 0.2e1 * k) + (0.9e1 / 0.2e1)) * p1)
                   ...
                   +                    (((((0.3e1 * k) - (0.11e2 / 0.4e1)) * k) + 0.1e1) *
                   p0) +                    (k * ((-0.3e1 / 0.2e1 * k) + (0.1e1 / 0.2e1)) *
              p3))) -               (0.9e1 / 0.8e1 * t2 * (p0 - p3)) + (0.27e2 / 0.8e1 * t2 * (p1 - p2));
    return grad;
}
...
inline std::pair<double, double> CubicSpiralBoundaryValueProblem::XY(
    ...
    const double resolution = 1.0 / static_cast<double>(kParams_.num_steps);
    ...
    for (int i = 0; i <= kParams_.num_steps; i++) {
        const double cur_ratio = static_cast<double>(i) * resolution;
...
double CubicSpiralBoundaryValueProblem::LeastSquare(const VecXd &vecx,
    ...
    *ptr_grad = (2.0 * kParams_.wx * dx * xgrad) +
                (2.0 * kParams_.wy * dy * ygrad) +
...
std::vector<Spiral> CubicSpiralBoundaryValueSolver::solve(
    ...
    if (std::fabs(theta1) <= kLargeAngleThreshold) {
        ...
        possible_thetas.push_back(theta1);
        possible_thetas.push_back(theta1 - (2.0 * M_PI));
    } else {
        possible_thetas.push_back(theta1 + (2.0 * M_PI));
        possible_thetas.push_back(theta1);
    }
    ...
    for (const auto &theta : possible_thetas) {
        CubicSpiralBoundaryValueProblem problem(k0, rotated_xy.x(),
                                                rotated_xy.y(), theta, k1);
