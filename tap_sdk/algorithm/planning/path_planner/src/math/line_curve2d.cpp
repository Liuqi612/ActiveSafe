...
#include "path_planner/src/math/line_curve2d.h"
#include "path_planner/src/common/log.h"
#include "path_planner/src/math/double.h"
#include "path_planner/src/math/linear_interpolation.h"
...
bool LineCurve2d::SamplePoints(const double start_s,
                               const double length,
    ...
    while ((Double::Compare(cur_s, sample_end_s) != CompareType::GREATER) &&
        ...
        if (idx == 0UL) {
            ...
            sample_pt = math::InterpolateUsingLinearApproximation(
                points_[idx - 1], points_[idx], w);
        }
...
double LineCurve2d::GetDistance(const Vec2d &point,
                                Vec2d *nearest_point,
                                double *s_offset) const {
    double min_dist = std::numeric_limits<double>::max();
    Vec2d nearest_pt;
    if (!IsValid()) return min_dist;
    for (std::size_t i = 0U; i < points_.size() - 1U; ++i) {
        const auto &start = points_[i];
        const auto &end = points_[i + 1U];
        const double seg_length = accumulated_s_[i + 1U] - accumulated_s_[i];
        const math::Vec2d unit_direction = (end - start) / seg_length;
        double dist_t = DBL_MAX;
        const double proj = unit_direction.InnerProd(point - start);
        if ((seg_length <= math::kMathEpsilon) || (proj < 0.0)) {
            nearest_pt = start;
        ...
        if (dist_t < min_dist) {
            ...
            if (s_offset) {
                *s_offset =
                    accumulated_s_[i] + points_[i].DistanceTo(nearest_pt);
            }
        }
    }
    return min_dist;
...
bool LineCurve2d::GetProjection(const Vec2d &point,
                                double *accumulate_s,
                                double *lateral) const {
    if (!IsValid() || (accumulate_s == nullptr) || (lateral == nullptr))
        return false;
    double min_dist = std::numeric_limits<double>::infinity();
    const std::size_t seg_num = points_.size() - 1U;
    std::size_t min_index = 0U;
    for (std::size_t i = 0U; i < seg_num; ++i) {
        const auto &start = points_[i];
        const auto &end = points_[i + 1U];
        const double seg_length = accumulated_s_[i + 1U] - accumulated_s_[i];
        if (math::Double::Compare(seg_length, 0.0) ==
    ...
    const auto &nearest_seg =
        math::LineSegment2d(points_[min_index], points_[min_index + 1U]);
...
bool LineCurve2d::GetPoint(const double s,
                           const double l,
    ...
    if (s < 0.0) {
        ...
        index = points_.size() - 1U - 1U;
    } else {
        std::size_t seg_num = points_.size() - 1U;
        index = seg_num - 1U;

        for (std::size_t i = 0U; i < seg_num; ++i) {
            if (accumulated_s_.at(i + 1U) > s) {
                index = i;
                break;
            }
        }
    ...
    const auto &nearest_seg =
        math::LineSegment2d(points_[index], points_[index + 1U]);
