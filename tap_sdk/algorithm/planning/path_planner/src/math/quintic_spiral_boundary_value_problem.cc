...
#include "path_planner/src/math/quintic_spiral_boundary_value_problem.h"
...
std::pair<double, double> XY(
    ...
    const double resolution = 1.0 / static_cast<double>(params.num_steps);
    ...
    for (int i = 0; i <= params.num_steps; i++) {
        ...
        const auto theta_grad =
            ThetaGrad(p0, p1, p2, p3, p4, p5, sg, cur_ratio);
...
std::vector<Spiral> QuinticSpiralBoundaryValueSolver::solve(
    ...
    for (const auto &theta : possible_thetas) {
        ...
        QuinticSpiralBoundaryValueProblem problem(
            sp0.k, sp1.x, sp1.y, sp1.theta, sp1.k, sp0.dk, sp0.ddk);
