...
class SegmentedSpeedLimitCostV2 : public Cost<PROB> {
 public:
     ...
     public:
        SpeedLimitInfo(const std::vector<double> &speed_limits,
            ...
            : speed_limits_(speed_limits),
              ...
              size_(static_cast<int>(speed_limits_.size())) {
            XCHECK_EQ(speed_limits_.size(),
                      additional_speed_limits_.size() + 1);
            for (const auto &speed_limit_info : additional_speed_limits) {
                for (int i = 1; i < speed_limit_info.size(); ++i) {
                    XCHECK_LE(speed_limit_info[i - 1].alpha,
                              speed_limit_info[i].alpha);
                }
            }
        }
        ...
        std::pair<SpeedlimitInfoPoint, SpeedlimitInfoPoint>
        GetSpeedLimitInfoPair(int index, double alpha) {
            XCHECK_GE(index, 0);
            XCHECK_LE(index, size_ - 1);
            const auto &additional_speed_limit =
                additional_speed_limits_[index];
            if (alpha < 0.0) {
                return {{0.0, speed_limits_[index]},
                        {0.0, speed_limits_[index]}};
            } else if (alpha >= 1.0) {
                ...
                if (additional_speed_limit.empty()) {
                    ...
                    if (alpha < additional_speed_limit.front().alpha) {
                        ...
                        const auto max_iter = std::upper_bound(
                            additional_speed_limit.begin(),
                            additional_speed_limit.end(), alpha,
                            [](double value, const SpeedlimitInfoPoint &pt) {
                                return value < pt.alpha;
                            });
                        const int index = static_cast<int>(std::distance(
                            additional_speed_limit.begin(), max_iter));
    ...
    explicit SegmentedSpeedLimitCostV2(
        ...
        SpeedLimitInfo speed_limits,
        SpeedLimitInfo free_speed_limits,
        int free_index,
        ...
        if (center_line_query_helper == nullptr) {
            ...
            XCHECK_EQ(ref_points.size(), speed_limits_.size());
            XCHECK_EQ(ref_points.size(), free_speed_limits_.size());
        } else {
            XCHECK_GT(center_line_query_helper->points().size(), 1);
            XCHECK_EQ(center_line_query_helper->points().size(),
                      speed_limits_.size());
            XCHECK_EQ(center_line_query_helper->points().size(),
    ...
    DividedG SumGForAllSteps(const StatesType &xs,
        ...
        for (int k = 0; k < horizon_; ++k) {
            res.AddSubG(0, gain_[k] * Sqr(speed_diff_[k]));
        }
    ...
    void Update(const StatesType &xs,
        ...
        if (center_line_query_helper_ == nullptr) {
            for (int k = 0; k < horizon_; ++k) {
                ...
                if (k <= free_index_) {
                    UpdateSpeedLimitInfo(
                        PROB::v(xs, k),
                        speed_limits_.GetSpeedLimitInfoPair(index, alpha),
                        lengths_inv_[index], alpha, normal, &speed_diff_[k],
                        &gain_[k], &speed_limit_factor_[k]);
                } else {
                    UpdateSpeedLimitInfo(
                        PROB::v(xs, k),
            ...
            const auto &rac_index_pairs =
                center_line_query_helper_->index_pairs();
    ...
    void UpdateSpeedLimitInfo(
        ...
        *speed_diff = speed_diff_tmp;
        ...
        if (length < kEps) {
            ...
            *speed_limit_factor = (speed_limit_info_next.speed_limit -
