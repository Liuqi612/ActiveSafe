...
/**
 * @brief This struct represents a gradient arrow.
 */
struct GradArrow {
  double px = 0.0;  // 箭头起点 x
  double py = 0.0;  // 箭头起点 y
  double dx = 0.0;  // 箭头方向 x
  double dy = 0.0;  // 箭头方向 y

  int circle_idx = -1;  // 圆心索引，-1 表示无圆心
  int step_idx = -1;    // 箭头所在步长索引，-1 表示无步长索引
};


struct GradArrowWithCost {
  GradArrow inner_arrow;
  GradArrow outer_arrow;
...
class Cost {
 public:
    ...
    virtual double EvaluateG(int k,
                             const StateType &x,
                             const ControlType &u) const = 0;

    virtual DividedG EvaluateGWithDebugInfo(int k,
                                            const StateType &x,
                                            const ControlType &u,
    ...
    virtual DGDxType EvaluateDGDx(int k,
                                  const StateType &x,
    ...
    virtual DGDuType EvaluateDGDu(int k,
                                  const StateType &x,
                                  const ControlType &u) const {
    ...
    virtual DDGDxDxType EvaluateDDGDxDx(int k,
                                        const StateType &x,
                                        const ControlType &u) const {
    ...
    virtual DDGDuDxType EvaluateDDGDuDx(int k,
                                        const StateType &x,
                                        const ControlType &u) const {
    ...
    virtual DDGDuDuType EvaluateDDGDuDu(int k,
                                        const StateType &x,
                                        const ControlType &u) const {
    ...
    virtual void AddDGDx(int k,
                         const StateType &x,
                         const ControlType &u,
                         DGDxType *dgdx) const = 0;
    virtual void AddDGDu(int k,
                         const StateType &x,
                         const ControlType &u,
                         DGDuType *dgdu) const = 0;

    virtual void AddDDGDxDx(int k,
                            const StateType &x,
                            const ControlType &u,
                            DDGDxDxType *ddgdxdx) const = 0;
    virtual void AddDDGDuDx(int k,
                            const StateType &x,
                            const ControlType &u,
                            DDGDuDxType *ddgdudx) const = 0;
    virtual void AddDDGDuDu(int k,
                            const StateType &x,
                            const ControlType &u,
                            DDGDuDuType *ddgdudu) const = 0;

    virtual void Update(const StatesType &xs,
                        const ControlsType &us,
                        int horizon) {}
  ...
  virtual GradArrowWithCost EvaluateBoundary(int horizon) const {
    return GradArrowWithCost();
  }
