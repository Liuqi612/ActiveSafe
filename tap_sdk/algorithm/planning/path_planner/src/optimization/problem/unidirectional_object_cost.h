...
class UnidirectionalObjectCost : public Cost<PROB> {
 public:
    ...
    explicit UnidirectionalObjectCost(
        ...
        std::string name_in = "UnidirectionalObjectCost",
        double scale_in = 1.0,
        ...
        for (int k = 0; k < num_objects_; ++k) {
            ...
            XCHECK_EQ(ob.buffers.size(), sub_names_.size());
            for (int i = 0; i < circle_size_; ++i) {
                penetrations_[k][i].resize(
                    ob.buffers.size(), std::numeric_limits<double>::infinity());
            }
        }
    }
    ...
    double EvaluateG(int k,
        ...
        for (int i = 0; i < circle_size_; ++i) {
            ...
            for (int j = 0; j < penetration_ki.size(); ++j) {
                if (penetration_ki[j] < 0.0) {
                    g += 0.5 * Cost<PROB>::scale() * ob.gains[j] *
                         Sqr(penetration_ki[j]);
                }
            }
        }
        return g;
    ...
    void AddDGDx(int k,
        ...
        for (int i = 0; i < circle_size_; ++i) {
            ...
            for (int j = 0; j < penetration_ki.size(); ++j) {
                if (penetration_ki[j] < 0.0) {
                    *dgdx += Cost<PROB>::scale() * ob.gains[j] *
                             penetration_ki[j] * penetration_jacobian;
                }
            }
        }
    }
    ...
    void Update(const StatesType &xs,
        ...
        for (int k = 0; k < horizon; ++k) {
            ...
            for (int i = 0; i < circle_size_; ++i) {
                ...
                if (lat_offset > ob.lateral_extent ||
                    lat_offset < -ob.lateral_extent) {
                    filtered_k[i] = true;
                    continue;
                }
    ...
    void UpdateDerivatives(const StatesType &xs,
        ...
        for (int k = 0; k < horizon; ++k) {
            ...
            for (int i = 0; i < circle_size_; ++i) {
                ...
                const Vec2d &dir = ob.dir;
                penetration_jacobian_ki.template segment<2>(
                    PROB::kStateXIndex) = dir.transpose();
