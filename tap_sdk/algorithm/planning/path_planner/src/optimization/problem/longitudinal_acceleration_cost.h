...
class LongitudinalAccelerationCost : public Cost<PROB> {
 public:
    ...
    explicit LongitudinalAccelerationCost(
        double max_acceleration_buffer,
        double max_deceleration_buffer,
        ...
        XCHECK_GT(max_acceleration_buffer_, max_deceleration_buffer_);
        XCHECK_EQ(accel_cascade_buffers_[0], 0.0);
        XCHECK_EQ(decel_cascade_buffers_[0], 0.0);
        XCHECK_EQ(accel_cascade_buffers_.size(), accel_cascade_gains_.size());
        XCHECK_EQ(decel_cascade_buffers_.size(), decel_cascade_gains_.size());
    }
