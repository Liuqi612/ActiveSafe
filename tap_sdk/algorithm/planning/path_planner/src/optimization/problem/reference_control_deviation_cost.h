...
class ReferenceControlDeviationCost : public Cost<PROB> {
 public:
    ...
    ReferenceControlDeviationCost(
        ...
        : Cost<PROB>(std::move(name), scale * kNormalizedScale, cost_type),
          ...
          weights_(weights.empty() ? std::vector<double>(ref_us_.size(), 1.0)
                                   : std::move(weights)) {
        XCHECK_EQ(ref_us_.size(), weights_.size());
    }

    DividedG SumGForAllSteps(const StatesType &xs,
                             const ControlsType &us,
                             int horizon) const override {
        XCHECK_EQ(us.rows(), ref_us_.rows());
        DividedG res(1);
        res.SetSubName(0, Cost<PROB>::name());
        for (int k = 0; k < horizon; k++) {
            const ControlType u_diff = PROB::GetControlAtStep(us, k) -
                                       PROB::GetControlAtStep(ref_us_, k);
            res.AddSubG(
                0, (Sqr(u_diff[0]) * weights_[(k * PROB::kControlSize) + 0]) +
    ...
    void AddDGDu(int k,
        ...
        const ControlType u_diff = u - PROB::GetControlAtStep(ref_us_, k);
        (*dgdu)[0] += Cost<PROB>::scale() * u_diff[0] *
                      weights_[(k * PROB::kControlSize) + 0];
        (*dgdu)[1] += Cost<PROB>::scale() * u_diff[1] *
                      weights_[(k * PROB::kControlSize) + 1];
    }
