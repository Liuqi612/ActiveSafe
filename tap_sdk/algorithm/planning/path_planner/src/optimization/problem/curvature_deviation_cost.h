...
class CurvatureDeviationCost : public Cost<PROB> {
 public:
    ...
    CurvatureDeviationCost(std::vector<double> ref_path_s,
        ...
        XCHECK_EQ(ref_path_s_.size(), ref_path_kappa_.size());
    ...
    DividedG SumGForAllSteps(const StatesType &xs,
        ...
        for (int k = 0; k < horizon; ++k) {
            res.AddSubG(0, weights_[k] * Sqr(kappa_diff_[k]));
        }
    ...
    void Update(const StatesType &xs,
        ...
        for (int i = 0; i < horizon; ++i) {
            const std::vector<double>::difference_type index_distance = std::distance(
                ref_path_s_.begin(),
                std::upper_bound(ref_path_s_.begin(), ref_path_s_.end(),
                                 PROB::s(xs, i)));
            int index = static_cast<int>(index_distance);
            ...
            if (index == ref_path_s_.size() - 1) {
                ...
                if (std::abs(s_base) < 1e-9) {
                    ...
                    factor_[i] =
                        (ref_path_kappa_[index + 1] - ref_path_kappa_[index]) /
                        s_base;
                }
                kappa_diff_[i] =
                    PROB::kappa(xs, i) - ((alpha * (ref_path_kappa_[index + 1] -
