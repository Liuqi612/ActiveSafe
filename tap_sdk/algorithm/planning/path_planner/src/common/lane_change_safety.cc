...
double FLAGS_planner_lc_safety_radical_factor = 0.8;
double FLAGS_planner_lc_safety_conservative_factor = 1.0;

constexpr double kEnterTargetLateralThreshold = 1.5;
constexpr double kFullyEnterTargetLateralThreshold = 1.2;
constexpr double kStaticEnterTargetLateralThreshold = 0.8;
...
const std::vector<double> LON_OVERLAP_VEC = {-6.0, -4.0, -2.0, 0.0};
const std::vector<double> LATERAL_OVERLAP_VEC = {-1.8, -2.5, -3.0, -3.8};
...
const std::vector<double> MIN_IGNORE_OBJ_DIST_VEC = {1.0, 1.5, 2.5,
                                                     3.5, 5.0, 6.0};
const std::vector<double> V_EGO_IGNORE_OBJ_DIST_VEC = {30, 40, 50, 60, 80, 100};
...
const double kMinCompensationFactor = 0.28;
...
std::vector<double> occupied_width_vec = {1.0, 0.9, 0.8, 0.7, 0.6};
std::vector<double> rel_distance_vec = {0.0, 30, 60, 80, 120};

const std::vector<double> MIN_ACC_TTC_VEC = {2.8, 3.2, 3.4, 3.6, 3.8, 4.0};
const std::vector<double> V_EGO_ACC_TTC_VEC = {55, 70, 85, 100, 110, 120};
const double kDistBufferInAccTTC = 0.5;
...
bool HasEnteredTargetLane(const FrenetBox &obj_box, const double lane_width) {
    ...
    if (obj_box.l_max * obj_box.l_min > 0.0) {
        double min_dis_target =
            std::min(std::fabs(obj_box.l_max), std::fabs(obj_box.l_min));
        double intrusion_dis_thrd = boost::algorithm::clamp(
            (0.38 * (lane_width / kDefaultLaneWidth)), 0.2, 0.5);
...
bool isEgoFullyOccupyTargetLane(const FrenetBox &ego_frenet_box,
    ...
    double enter_target_lane_dis_thrd = ads_x::planning::math::interp1_inc(
...
bool isStaticObsRideLine(const FrenetBox &ego_box,
                         const FrenetBox &obj_box,
                         const bool lc_left,
                         const double rel_ds) {
    double occupied_width_limit = ads_x::planning::math::interp1_inc(
...
bool isStaticObsOccupyTargetLane(
    ...
    bool big_heading =
        (max_dis_target - min_dis_target) / (2 * half_width) > 1.2;

    bool far_away_ego_obs =
        (obj_box.l_max * obj_box.l_min > 0.0) &&
    ...
    if (obj_box.l_max * obj_box.l_min > 0.0) {
        bool close_ego_side_obs =
            lc_left ? (obj_box.l_max < 0.0) : (obj_box.l_min > 0.0);
        corner_has_entered =
...
inline bool IsBigVehicle(
    ...
    double obs_width = !obstacle_trajectory->states().empty()
                           ...
                           : 2.0;
    bool is_big_vehicle =
        (obs_type == OT_LARGE_VEHICLE && (obs_length > 6.0 || obs_width > 2.6 ||
                                          obs_length * obs_width > 11.6));
...
inline bool IsSizeQualified(
    ...
    constexpr double kMinQualifiedDimension = 0.3;
    constexpr double kMinQualifiedVolume = 0.003;
    if ((obs_length >= kMinQualifiedDimension) ||
        (obs_width >= kMinQualifiedDimension) ||
        (obs_height >= kMinQualifiedDimension) ||
        (obs_length * obs_width * obs_height >= kMinQualifiedVolume)) {
        size_qualified = true;
    }
...
inline double getMinDisCompensation(bool if_obs_front,
                                    bool is_big_vehicle,
    ...
    if (frt_big_veh) {
        ...
        double factor = std::floor(obj_cur_box.length() /
                                   ((2 * ego_veh_length) + kEpsilon));
        min_dis_compensation =
            boost::algorithm::clamp((factor * 0.5), 0.0, 3.0);
    }

    return min_dis_compensation;
}

inline double QuadraticLerp(
    double x0, double t0, double x1, double t1, double t, bool clamp = true) {
...
double getLongOverlap(const pnc_x::FrenetBox &ego_cur_frenet_box,
                      const pnc_x::FrenetBox &obj_cur_frenet_box,
                      double &rel_ds) {
    ...
    rel_ds =
        (long_overlap >= 0.0) ? 0.0 : (if_obs_front ? -long_overlap : long_overlap);
    return long_overlap;
}

double getLatOverlap(const pnc_x::FrenetBox &ego_cur_frenet_box,
                     const pnc_x::FrenetBox &obj_cur_frenet_box,
                     double &rel_dl) {
    ...
    rel_dl =
        (lat_overlap >= 0.0) ? 0.0 : (if_obs_left ? -lat_overlap : lat_overlap);
    return lat_overlap;
};

double getLatOverlapWithLcDirection(const pnc_x::FrenetBox &ego_cur_frenet_box,
                                    const pnc_x::FrenetBox &obj_cur_frenet_box,
                                    double &rel_dl) {
    const double kLcDirectionMaxSpacingDis = 3.75;
    const bool lc_left = ego_cur_frenet_box.center_l() < 0.0;
    double lat_overlap_with_direction = 0.0;
    const bool if_obs_left =
        (ego_cur_frenet_box.l_max < obj_cur_frenet_box.l_min);
    const double lat_overlap =
        std::min(ego_cur_frenet_box.l_max, obj_cur_frenet_box.l_max) -
    ...
    if (lc_left &&
        ((lat_overlap >= 0.0) ||
         ((rel_dl > 0.0) && ((obj_cur_frenet_box.l_min - ego_cur_frenet_box.l_max) <
                            kLcDirectionMaxSpacingDis)))) {
        lat_overlap_with_direction =
            obj_cur_frenet_box.l_max - ego_cur_frenet_box.l_min;
    } else if (!lc_left && ((lat_overlap >= 0.0) ||
                            ((rel_dl < 0.0) && (ego_cur_frenet_box.l_min -
                                                   obj_cur_frenet_box.l_max <
                                               kLcDirectionMaxSpacingDis)))) {
        lat_overlap_with_direction =
            ego_cur_frenet_box.l_max - obj_cur_frenet_box.l_min;
    } else {
        lat_overlap_with_direction = 0.0;
    }
...
double getLatOffsetFactor(const pnc_x::FrenetBox &ego_cur_frenet_box,
...
double getLatOffsetFactor(const pnc_x::FrenetBox &ego_cur_frenet_box,
                          double ego_half_width,
                          double min_compensation_factor,
                          double intrusion_dis) {
    const bool lc_left = ego_cur_frenet_box.center_l() < 0.0;
    const double closet_corner_lat_offset =
        lc_left ? std::abs(std::min(0.0, ego_cur_frenet_box.l_max))
...
inline bool IsLatClosestBlocking(const FrenetBox &ego_box,
    ...
    return !HasFullyEnteredTargetLane(obj_box, 0.5 * obj_box.width()) &&
           ...
           ((!lc_left &&
             (ego_box.center_l() - ego_box.width() * 0.5 -
                      obj_box.width() * 0.5 -
                      std::max(kMinLatBufferToCloseBlocking,
                               kLatThresholdToCloseBlocking + obj_lat_ext) <
                  obj_box.center_l())) ||
            (lc_left &&
             ((obj_box.center_l() - ego_box.width() * 0.5 -
                             obj_box.width() * 0.5 -
                             std::max(kMinLatBufferToCloseBlocking,
                                      kLatThresholdToCloseBlocking -
                                          obj_lat_ext)) < ego_box.center_l())));
}
    ...
    for (size_t i = 1; i < obj_states.size(); ++i) {
        ...
        return LerpBox2d(
            ...
            (start_time_offset - obj_states[i - 1].traj_point->t()) /
                (obj_state.traj_point->t() -
                 obj_states[i - 1].traj_point->t()));
    }
...
double EstimateObjectSpeed(const PlannerObject &object, double preview_time) {
    ...
    if (!accel_hist.empty()) {
        constexpr size_t kMaxConsideredAccelHistoryItem = 5;
...
bool IsLeavingTargetLanePath(const FrenetFrame &target_frenet_frame,
    ...
    constexpr double kLeaveLaneLatRatio = 0.10;
...
bool ObjIsLeavingTargetLane(const FrenetBox &obj_cur_box,
                            const pnc_x::FrenetCoordinate &obj_preview_pnt) {
    const double obj_width = obj_cur_box.width();
    const double obj_cur_l = obj_cur_box.center_l();
...
bool PathHasOverlap(absl::Span<const Box2d> ego_boxes,
                    double obj_v,
                    absl::Span<const SpacetimeObjectState> obj_states) {
...
double ComputeMinLonBufferSimilarSpeedFactor(
    ...
    double k_speed_factor = ads_x::planning::math::interp1_inc(
        V_FOLLOW_DV_VEC,
        (!is_motor ? K_SPEED_FACTOR_AGGR_VEC : K_SPEED_FACTOR_CNSV_VEC),
        Mps2Kph(dv_to_follow));

    return boost::algorithm::clamp(k_speed_factor, 0.7, 2.0);
}
...
double ComputeSimilarSpeedFactor(double lead_v, double follow_v, double ref_v) {
    ...
    return boost::algorithm::clamp(
        (follow_v - lead_v + SpeedDiffThres) / SpeedDiffThres, 0.0, 1.0);
}

double ComputeEgoLeadTime(double speed_limit,
                          double ego_v,
                          double obj_v,
                          LaneChangeStage lc_state) {
    double k_speed_factor = ads_x::planning::math::interp1_inc(
    ...
    double ego_stationary_time_compensation = 0.0;
    if (ego_v < kEpsilon) {
        ...
        ego_stationary_time_compensation = 0.0;
    }
    ...
    const double min_lead_time =
        (obj_v != 0.0) ? g_MinLonBuffer / (obj_v + kEpsilon) : 0.0;
...
double ComputeEgoFollowTime(double obj_v,
                            double ego_v,
                            LaneChangeStage lc_state) {
    double k_speed_factor = ads_x::planning::math::interp1_inc(
    ...
    const double min_follow_time =
        (ego_v != 0.0) ? g_MinLonBuffer / (ego_v + kEpsilon) : 0.0;

    if (obj_v > ego_v * kHigherSpeedThresRatio && ego_v != 0.0) {
    ...
    return std::max(min_follow_time,
           ...
           PauseFactor * k_speed_factor;
}

absl::Status CheckDeceleration(double lon_dist,
                               std::string name_lead,
                               std::string name_follow,
                               double v_lead,
                               double v_follow,
                               double response_time,
                               double lead_time,
                               double max_allowed_decel,
    ...
    if (buffered_lon_dist <= kEpsilon) {
        return absl::CancelledError(absl::StrFormat(
            ...
            name_follow, name_lead, lon_dist, lead_time, v_lead, v_follow,
            v_diff, max_allowed_decel, g_MinLonBuffer, acc_compensation_dis));
    }
    ...
    if (max_decel != nullptr && hypo_decel > *max_decel)
        *max_decel = hypo_decel;
...
absl::Status CheckSafetyByAccTTC(double v_lead,
                                 double v_follow,
                                 double acc_lead,
                                 double acc_follow,
                                 double lon_dist,
                                 double lat_offset_factor,
                                 std::string *acc_ttc_debug_info) {
    if (Mps2Kph(v_follow) <= 55.0) {
        acc_ttc_debug_info->clear();
        *acc_ttc_debug_info = absl::StrFormat(
            "The speed is less than 55 km/h. "
            "v_lead:%.2f v_follow:%.2f v_min:%.2f",
            v_lead, v_follow, Mps2Kph(v_follow));
        return absl::OkStatus();
    }

    if (v_lead > v_follow && acc_lead > acc_follow) {
        acc_ttc_debug_info->clear();
        *acc_ttc_debug_info = absl::StrFormat(
            "The acc and velocity of the lead vehicle are both greater than "
            "the "
            "follow vehicle. "
            "v_lead:%.2f v_follow:%.2f acc_lead:%.2f acc_follow:%.2f",
            v_lead, v_follow, acc_lead, acc_follow);
        return absl::OkStatus();
    }

    constexpr double kMinAttentionDistInAccTTC = 30.0;
    if (std::abs(lon_dist) > kMinAttentionDistInAccTTC) {
        acc_ttc_debug_info->clear();
        *acc_ttc_debug_info = absl::StrFormat(
            "The lon distance between the lead vehicle and the follow vehicle "
            "is greater than threshold. "
            "lon_dist:%.2f threshold:%.2f",
            lon_dist, kMinAttentionDistInAccTTC);
        return absl::OkStatus();
    }

    const double A = 0.5 * (acc_lead - acc_follow);
    const double B = v_lead - v_follow;
    const double C = lon_dist + kDistBufferInAccTTC;
    const double D = B * B - 4 * A * C;

    if (D < 0.0) {
        acc_ttc_debug_info->clear();
        *acc_ttc_debug_info = absl::StrFormat(
            "D < 0.0. v_lead:%.2f v_follow:%.2f acc_lead:%.2f acc_follow:%.2f "
            "lon_dist:%.2f D:%.2f",
            v_lead, v_follow, acc_lead, acc_follow, lon_dist, D);
        return absl::OkStatus();
    }

    const double t_1 = (-B - std::sqrt(D)) / (2 * A + kEpsilon);
    const double t_2 = (-B + std::sqrt(D)) / (2 * A + kEpsilon);
    const double ttc_threshold =
        ads_x::planning::math::interp1_inc(V_EGO_ACC_TTC_VEC, MIN_ACC_TTC_VEC,
                                           Mps2Kph(v_follow)) -
        (1.0 - lat_offset_factor);
    double acc_ttc = 0.0;
    if (t_1 < 0.0 && t_2 < 0.0) {
        acc_ttc = 100.0;
    ...
    if (acc_ttc < ttc_threshold) {
        acc_ttc_debug_info->clear();
        *acc_ttc_debug_info = absl::StrFormat(
            "The follow vehicle will hit the lead vehicle. "
            "v_lead:%.2f v_follow:%.2f acc_lead:%.2f acc_follow:%.2f "
            "lon_dist:%.2f t_1:%.2f t_2:%.2f acc_ttc:%.2f ttcthreshold:%.2f",
            v_lead, v_follow, acc_lead, acc_follow, lon_dist, t_1, t_2, acc_ttc,
            ttc_threshold);
        return absl::CancelledError(acc_ttc_debug_info->c_str());
    }
    acc_ttc_debug_info->clear();
    *acc_ttc_debug_info = absl::StrFormat(
        "safe by acc_ttc: "
...
double getAccelerationDistanceCompensation(double offset_factor,
                                           double ego_acc,
                                           double obs_acc,
                                           double rel_vs,
                                           bool if_obs_front,
    ...
    double rel_acc_temp = boost::algorithm::clamp(rel_acc, -5.0, 2.0);
    double acc_safe_dis = 0.0;
    double pred_time = 3.0;
    double res_sign = 0.0;
    ...
    if (if_obs_front) {
        if (rel_acc_temp > 0.0) {
            res_sign = 1.0;
        } else if (rel_acc_temp < 0.0 && ego_acc < 0.5) {
            res_sign = -1.0;
        } else {
            res_sign = 0.0;
        }
    ...
    if (res_sign == 1.0) {
        if (is_distance_shortening && !if_obs_front &&
            ((rel_vs > 3.0 && (ego_acc < -0.5 || obs_acc > 0.8)) ||
             ego_acc < -1.5)) {
...
bool isLaneChanging(const pnc_x::LaneChangeStage &lc_state) {
    const bool is_lane_change_state =
        (lc_state == LaneChangeStage::LCS_EXECUTING) ||
        (lc_state == LaneChangeStage::LCS_PAUSE) ||
        (lc_state == LaneChangeStage::LCS_RETURN);
...
absl::Status CheckLaneChangeSafety(
    ...
    constexpr int kRideLineUuCountThreshold = 3;
    ...
    double kEgoLeadTimeBuffer = is_lc_state_prev ? kEgoLeadTimeBufferProcess
                                                 : kEgoLeadTimeBufferPrepare;
    double kEgoFollowTimeBuffer = is_lc_state_prev
                                      ? kEgoFollowTimeBufferProcess
                                      : kEgoFollowTimeBufferPrepare;
    ...
    ego_boxes.reserve(static_cast<size_t>(ego_traj_size));
    ...
    for (int i = 0; i < ego_traj_size; ++i) {
        const auto &traj_pt = ego_traj_pts[static_cast<size_t>(i)];
    ...
    constexpr int kMustEnterTargetStep =
        static_cast<int>(0.95 * static_cast<double>(kTrajectorySteps));
    if ((ego_enter_target_idx == -1 ||
    ...
    auto ego_front_vct =
        std::vector<pnc_x::Vec2d>{ego_corner.front(), ego_corner.back()};
    ASSIGN_OR_RETURN(
    ...
    const auto ego_target_tangent_cos =
        ego_cur_box.tangent().Dot(target_frenet_frame.InterpolateTangentByS(
            ego_cur_frenet_box.center_s()));
    ...
    double conserv_base =
        (FLAGS_planner_enable_lc_style_params
    ...
    double temp_min_front_dist = ads_x::planning::math::interp1_inc(
        V_EGO_VEHICLE_VEC, MIN_FRONT_DISTANCE_VEC, ego_speed_kph);
    double temp_min_back_dist = ads_x::planning::math::interp1_inc(
        V_EGO_VEHICLE_VEC, MIN_BACK_DISTANCE_VEC, ego_speed_kph);

    const double ego_enter_target_time =
        static_cast<double>(ego_enter_target_idx) * kTrajectoryTimeStep;
    ...
    double pause_factor =
        (prev_lc_stage == LaneChangeStage::LCS_PAUSE) ? 1.5 : 1.0;

    pnc_x::FrenetBox frnt_static_filter_obs_box{DBL_MAX, DBL_MAX, DBL_MAX,
                                                DBL_MAX};
    std::string frnt_static_filter_obs_id = "";

    pnc_x::FrenetBox frnt_obs_box{DBL_MAX, DBL_MAX, DBL_MAX, DBL_MAX};
    pnc_x::FrenetBox rear_obs_box{DBL_MAX, DBL_MAX, DBL_MAX, DBL_MAX};
    ...
    for (const auto *traj_ptr : st_traj_mgr.moving_object_trajs()) {
        ...
        if (!XCHECK(!traj_ptr->states().empty())) {
            return absl::CancelledError("traj_ptr->states().empty()");
        }
        ASSIGN_OR_CONTINUE(
        ...
        bool is_distance_shortening = rel_ds * rel_vs < 0.0;
        ...
        bool is_ego_extra_slow =
            (ego_speed_kph < kExtremelySlowVehicleSpeedkph);
        ...
        const auto obj_target_normal_cos = obj_cur_box.tangent().Dot(
            target_frenet_frame
                .InterpolateTangentByS(obj_cur_frenet_box.center_s())
                .Rotate(M_PI_2));
        ...
        const double obj_enter_target_time = boost::algorithm::clamp(
            ComputeEnterTargetTime(target_frenet_frame, traj_ptr->states()),
            0.0, 255.0);
        ...
        conserv = conserv_base * pause_factor *
                  getObjTypeFactor(traj_ptr, if_obs_front);
        const double follow_obj_resp_time =
            conserv * kFollowerStandardResponseTime;

        double min_lon_buffer_factor = boost::algorithm::clamp(
        ...
        bool veh_will_stop = (obj_v < kExtremelySlowVehicleSpeedkph) ||
                             ((obs_acc < 0.0) && (-obj_v / obs_acc < 2.0) &&
                              (-obj_v / obs_acc >= 0.0));
        ...
        if (is_ego_stationary && !if_obs_front && is_rear_approaching_vehicle &&
            ...
            auto lat_overlap_safe_dis = ads_x::planning::math::interp1_inc(
                lat_threshold, lon_dis, lat_overlap_lc);

            double lat_overlap_factor = boost::algorithm::clamp(
                lat_overlap_lc / vehicle_geom.width(), 0.0, 2.0);
            ...
            auto front_safe_dis =
                std::max(lat_overlap_safe_dis, ego_v * 3.5 * conserv_base *
                                                   lat_overlap_factor *
                                                   state_debounce_factor);
            ...
            if (obj_danger) {
                ...
                return absl::CancelledError(absl::StrFormat(
                    ...
                    front_safe_dis, rel_ds, conserv_base, lat_overlap_safe_dis,
                    ego_v));
            }
        }
        ...
        const auto lon_safe_min_dist = ads_x::planning::math::interp1_inc(
        ...
        double acc_safe_dis = 0.0;
        if (!(obj_v < 0.0) && !(ego_v < 0.0) && !((ego_v == 0.0) && if_obs_front)) {
        ...
        AD_LINFO(PP) << "object_id()" << traj_ptr->object_id();
        ...
        auto obj_abreast_str = absl::StrFormat(
            ...
            min_lon_buffer_factor, rel_head_way_time, follower_vel, rel_ds,
            rel_dl, ego_v, obj_v, ego_acc, obs_acc, acc_safe_dis,
            ego_target_tangent_cos, obj_target_tangent_cos,
            ego_target_normal_cos, obj_target_normal_cos,
        ...
        const double lat_overlap_threshold = ads_x::planning::math::interp1_inc(
            LON_OVERLAP_VEC, LATERAL_OVERLAP_VEC, long_overlap);
        if (!LON_OVERLAP_VEC.empty() &&
            (long_overlap >= LON_OVERLAP_VEC.front())) {
            dynamic_obs_debug << "-s: " << long_overlap << "-d: " << lat_overlap
                              << "-t: " << lat_overlap_threshold;
        }
        const double min_ignore_lon_dist = ads_x::planning::math::interp1_inc(
            V_EGO_IGNORE_OBJ_DIST_VEC, MIN_IGNORE_OBJ_DIST_VEC, ego_speed_kph);
        dynamic_obs_debug << " s: " << std::to_string(rel_ds) << " "
                          << " ig_s: " << std::to_string(min_ignore_lon_dist)
                          << " ";
        if (!is_vru && obs_in_target_lane && obj_is_leaving_target &&
            (std::fabs(rel_ds) >= min_ignore_lon_dist) &&
            (lc_left ? obj_cur_frenet_box.l_min > 0.0
                     : obj_cur_frenet_box.l_max < 0.0) &&
            (lat_overlap < -1.8) && (obj_lc_left == lc_left)) {
            dynamic_obs_debug << "-Lea1"
                              << "-s: " << long_overlap << "-d: " << lat_overlap
                              << "-t: " << lat_overlap_threshold;
            continue;
        }

        if ((std::fabs(rel_ds) >= min_ignore_lon_dist) &&
            IsLeavingTargetLanePath(target_frenet_frame, lc_left,
        ...
        double debounce_braking_deceleration = 0.0;
        ...
        if (obj_cur_frenet_box.s_min <= ego_cur_frenet_box.s_max) {
            ...
            if ((ego_v >= obj_v) &&
                ...
                double rel_ttc =
                    (obj_v > ego_v) ? -rel_ds / (obj_v - ego_v) : DBL_MAX;
                bool impending_collision_scene =
                ...
                double max_decel_for_rear_object =
                    ads_x::planning::math::interp1_inc(
                ...
                if (is_lc_state_prev) {
                    double acc_factor =
                        impending_collision_scene
                            ? ads_x::planning::math::interp1_inc(
                                  std::vector<double>{-2.0, -1.0, 0, 1.0, 1.5,
                                                      2.0, 3.0},
                                  std::vector<double>{1.6, 1.3, 1.0, 0, -0.3,
                                                      -0.6, -1.2},
                                  obs_acc)
                            : 1.0;

                    debounce_braking_deceleration = boost::algorithm::clamp(
                        ((1.0 - conserv_base) * 2.5 * acc_factor), -1.2, 1.5);
                ...
                if (is_big_vehicle) {
                    ...
                    double obs_abs_center_l =
                        std::fabs(obj_cur_frenet_box.center_l());
                    double obs_offset_factor =
                        ads_x::planning::math::interp1_inc(
                            std::vector<double>{0.0, 0.3, 0.5, 1.5, 2.0},
                            std::vector<double>{1.1, 1.2, 1.3, 1.5, 2.0},
                            obs_abs_center_l);
                ...
                max_decel_for_rear_object = boost::algorithm::clamp(
                    max_decel_for_rear_object / pause_factor, 0.6, 3.8);

                std::string dec_str = "";
                auto check_status = CheckDeceleration(
                    std::fabs(rel_ds), "ego",
                    std::string(traj_ptr->object_id()),
                    (ego_v * ego_target_tangent_cos),
                    (obj_v * obj_target_tangent_cos), follow_obj_resp_time,
                    conserv * rel_head_way_time, max_decel_for_rear_object,
                    acc_safe_dis, dec_str, &follower_max_decel);
                dynamic_obs_debug << "-dec:" << !check_status.ok()
                                  << "-ttc:" << rel_ttc;

                double lc_in_the_same_time = is_lc_state_prev ? 3.5 : 4.0;
                if (obj_pred_to_target && !obs_in_target_lane &&
                    !ego_centered_in_target_lane &&
                    (rel_ttc < lc_in_the_same_time)) {
                    dynamic_obs_debug << "-lct:" << lc_in_the_same_time;
                    leader_debug_info << static_obs_debug.str();
                    follower_debug_info << dynamic_obs_debug.str();
                    return absl::CancelledError(check_status.ToString());
                }
                std::string acc_ttc_str = "";
                auto acc_ttc_status = CheckSafetyByAccTTC(
                    (ego_v * ego_target_tangent_cos),
                    (obj_v * obj_target_tangent_cos), ego_acc, obs_acc,
                    std::fabs(rel_ds),
                    getLatOffsetFactor(ego_cur_frenet_box, ego_half_width, 0.2),
                    &acc_ttc_str);
                ...
                if (rear_obs_id == traj_ptr->object_id()) {
                    ...
                    follower_debug_info << "---ACC_TTC:"
                                        << " obj id: " << traj_ptr->object_id()
                                        << " " << acc_ttc_str;
                }
                ...
                if (!acc_ttc_status.ok()) {
                    unsafe_object_id = traj_ptr->object_id();
                    status_code =
                        obs_in_target_lane
                            ? (if_obs_front
                                   ? PlannerStatusProto::
                    ...
                    leader_debug_info << static_obs_debug.str();
                    follower_debug_info << dynamic_obs_debug.str();
                    return absl::CancelledError(acc_ttc_status.ToString());
                }
            }
        } else {
            ...
            if ((ego_v <= obj_v) &&
                ...
                double max_decel_for_front_object =
                    ads_x::planning::math::interp1_inc(
                        DV_TTC_VEC, F_DV_FRONT_DEC_VEC, abs_rel_vs_kph);

                double rel_ttc =
                    (ego_v > obj_v) ? rel_ds / (ego_v - obj_v) : DBL_MAX;
                bool impending_collision_scene =
                ...
                if (is_lc_state_prev) {
                    double acc_factor =
                        impending_collision_scene
                            ? ads_x::planning::math::interp1_inc(
                                  std::vector<double>{-3.0, -2.0, -1.5 - 1.0, 0,
                                                      0.5, 1.0, 2.0},
                                  std::vector<double>{-1.0, -0.8, -0.5, 0, 1.0,
                                                      1.2, 1.3, 1.6},
                                  obs_acc)
                            : 1.0;

                    debounce_braking_deceleration = boost::algorithm::clamp(
                        ((1.0 - conserv_base) * 2.0 * acc_factor), -1.5, 2.0);
                ...
                } else if (((obs_acc < std::min(-1.0, ego_acc)) ||
                            (ego_acc > 0.1)) &&
                ...
                max_decel_for_front_object = boost::algorithm::clamp(
                    max_decel_for_front_object / pause_factor, 1.0, 5.0);

                std::string dec_str = "";
                auto check_status = CheckDeceleration(
                    std::fabs(rel_ds), std::string(traj_ptr->object_id()),
                    "ego", (obj_v * obj_target_tangent_cos),
                    (ego_v * ego_target_tangent_cos), kEgoResponseTime,
                    conserv * rel_head_way_time, max_decel_for_front_object,
                    acc_safe_dis, dec_str, &leader_max_decel);
                dynamic_obs_debug << "-dec:" << check_status.ok()
                                  << "-ttc:" << rel_ttc;

                std::string acc_ttc_str = "";
                auto acc_ttc_status = CheckSafetyByAccTTC(
                    (obj_v * obj_target_tangent_cos),
                    (ego_v * ego_target_tangent_cos), obs_acc, ego_acc,
                    std::fabs(rel_ds),
                    getLatOffsetFactor(ego_cur_frenet_box, ego_half_width, 0.2),
                    &acc_ttc_str);
        ...
        if (is_distance_shortening) {
            ...
            const std::vector<double> K_BACK_TTC_VEC = {0.83, 0.86, 0.90, 0.93,
                                                        0.96, 1.0,  1.2};
            const std::vector<double> V_TTC_VEC = {0.0, 15, 20, 30,
                                                   60,  90, 120};
            double front_ttc = 2.0, back_ttc = 2.0;
            bool quick_response_type = false;
            bool is_jam_scen = false;
            double spd_diff_kph_threshold = 100.0;
            if (is_distance_shortening) {
                if (if_obs_front) {
                    if (quick_response_type) {
                        spd_diff_kph_threshold = 100.0;
                    } else {
                        spd_diff_kph_threshold = 40.0;
                    }
                } else {
                    if (is_ego_stationary) {
                        spd_diff_kph_threshold = 8.0;
                    } else if (is_ego_extra_slow) {
                        spd_diff_kph_threshold = 10.0;
                    } else {
                        if (quick_response_type) {
                            spd_diff_kph_threshold = 30.0;
                        } else {
                            spd_diff_kph_threshold = 15.0;
                        }
                    }
                }
                if (is_lc_state_prev) spd_diff_kph_threshold += 5.0;
            } else {
                spd_diff_kph_threshold = 100.0;
            }
            ...
            bool large_spd_diff_scene =
                (is_distance_shortening &&
                 (abs_rel_vs_kph > (spd_diff_kph_threshold)));

            bool rear_large_spd_acc_scene =
                (is_distance_shortening &&
                 (abs_rel_vs_kph > (spd_diff_kph_threshold)*0.8) &&
                 (obs_acc > 2.0) && !if_obs_front);
            ...
            if (!is_rear_big_approaching_vehicle && !is_ego_extra_slow &&
                ...
                double k_front_ttc = ads_x::planning::math::interp1_inc(
                    V_TTC_VEC, K_FRONT_TTC_VEC, ego_speed_kph);

                double k_back_ttc = ads_x::planning::math::interp1_inc(
                ...
                std::vector<double> dv_ttc_vec = {0.0, 10, 20, 40, 60, 120};

                double f_dv_front_ttc = std::max(
                    ads_x::planning::math::interp1_inc(
                        dv_ttc_vec, f_dv_front_ttc_vec, abs_rel_vs_kph),
                    front_ttc);

                double f_dv_back_ttc =
                    std::max(ads_x::planning::math::interp1_inc(
                                 dv_ttc_vec, f_dv_back_ttc_vec, abs_rel_vs_kph),
                             back_ttc);

                front_ttc = std::max(k_front_ttc * f_dv_front_ttc,
                                     kLaneChangeMinLimitTTC);
                back_ttc = std::max(k_back_ttc * f_dv_back_ttc,
                                    kLaneChangeMinLimitTTC);
            ...
            const double kMinTTC = prev_lc_stage == LaneChangeStage::LCS_PAUSE
                                       ? 3.0 * pause_factor
                                       : 2.5;

            bool ttc_dis_danger =
                (std::fabs(rel_ds) < ttc_distance) || (ttc_rel <= kMinTTC);
            dynamic_obs_debug << "-ttc:" << ttc_dis_danger;
            auto ttc_debug_str = absl::StrFormat(
                "/***/Check_ttc:%d. obstacle %s will collide with ego after "
                "%.3f "
    ...
    pnc_x::FrenetBox frnt_static_current{DBL_MAX, DBL_MAX, DBL_MAX, DBL_MAX};
    pnc_x::FrenetBox frnt_static_target{DBL_MAX, DBL_MAX, DBL_MAX, DBL_MAX};
    ...
    for (const auto *traj_ptr : st_traj_mgr.stationary_object_trajs()) {
        ...
        const bool if_obs_front =
            (ego_cur_frenet_box.s_max < obj_frenet_box.s_min);
        const bool if_obs_rear =
            (obj_frenet_box.s_max < ego_cur_frenet_box.s_min);
        ...
        if ((rel_ds < -6.0) || (rel_ds > 135.0) ||
            (lc_left ? rel_dl < -0.5 : rel_dl > 0.5) ||
            (std::fabs(rel_dl) > 6.0)) {
        ...
        bool obs_in_target_lane = HasFullyEnteredTargetLane(
            obj_frenet_box, 0.5 * obj_frenet_box.width());
        ...
        conserv = boost::algorithm::clamp(conserv, 0.5, 1.0);
        ...
        double frt_obj_rel_ds, frt_obj_rel_dl = 0.0;
        ...
        if (!frnt_static_filter_obs_id.empty() &&
            ...
            auto frt_obj_lat_overlap = getLatOverlap(
                frnt_static_filter_obs_box, obj_frenet_box, frt_obj_rel_dl);
        }

        if (!frnt_static_filter_obs_id.empty() && target_lane_construction &&
            (frt_obj_rel_ds >= 0.0) && (std::fabs(frt_obj_rel_dl) < 0.3)) {
        ...
        bool occupy_current_condition =
            ...
            (is_vehicle || is_construction) && if_obs_front && (rel_dl == 0.0) &&
        ...
        if (occupy_current_condition || ride_line_condition) {
            ...
            double lat_overlap_factor = boost::algorithm::clamp(
                lat_overlap_lc <= 1.5 * vehicle_geom.width()
                    ? (lat_overlap_lc / vehicle_geom.width())
                    : (lat_overlap_lc + vehicle_geom.width()) /
                          (2.0 * vehicle_geom.width()),
                0.0, 2.5);
            ...
            auto front_safe_dis = std::max(
                ...
                ego_v * ((is_vehicle && if_obs_front) ? 3.5 : 3.0) *
                    conserv_base * lat_overlap_factor * state_debounce_factor);
            ...
            if (front_obs_too_close) {
                ...
                if (is_construction) {
                    status_code =
                        PlannerStatusProto::DANGEROUS_STATIONARY_OBSTACLE;
                } else {
                    status_code = PlannerStatusProto::
                        FRONT_DANGEROUS_VEHICLE_CURRENT_LANE;
                }
            ...
            if (static_occupy_target) {
                ...
                leader_debug_info << static_obs_debug.str();
                return absl::CancelledError(absl::StrFormat(
                    "UU or cone %s currently occupy target lane.",
                    traj_ptr->object_id()));
            }
        } else if ((ride_line_uu_num >= kRideLineUuCountThreshold) &&
                   ride_line_condition) {
            unsafe_object_id = std::string(traj_ptr->object_id());
            status_code = PlannerStatusProto::DANGEROUS_STATIONARY_OBSTACLE;
            leader_debug_info << static_obs_debug.str();
            return absl::CancelledError(absl::StrFormat(
                "Many uu or cone static obstacles ride line %s. %s",
        ...
        if ((obj_frenet_box.s_max < ego_cur_frenet_box.s_min) ||
            (std::abs(obj_frenet_box.center_l()) > 0.5 * obj_frenet_box.width() +
                                                      ego_half_width +
                                                      kStationaryBuffer)) {
        ...
        for (int i = 0; i < ego_traj_size; ++i) {
            if ((is_vehicle && static_occupy_target) ||
                ...
                static_obs_debug << "|overlap:" << i
                                 << "-occ:" << static_occupy_target;
                const double rel_head_way_time =
                    if_obs_front
                        ? ComputeEgoFollowTime(obj_v, ego_v, prev_lc_stage)
                ...
                const double obj_front_extension = conserv * follower_vel *
                                                   rel_head_way_time *
                                                   ego_target_tangent_cos;
                const double obj_lat_extension = -conserv * follower_vel *
                                                 rel_head_way_time *
                                                 ego_target_normal_cos;
                ...
                bool ego_is_entered =
                    ((ego_enter_target_idx == 0) && ego_occupy_target);
                ...
                const double lon_dist =
                    obj_frenet_box.s_min - ego_cur_frenet_box.s_max;
                ...
                if (is_lc_state_prev) {
                    double debounce_braking_deceleration =
                        boost::algorithm::clamp(((1.0 - conserv_base) * 2.0),
                                                0.0, 1.5);
                    max_decel_for_front_object += debounce_braking_deceleration;
                    max_decel_for_front_object = boost::algorithm::clamp(
                        max_decel_for_front_object, 0.0, 5.0);
                }
                ...
                std::string dec_str = "";
                auto check_status = CheckDeceleration(
                    lon_dist, std::string(traj_ptr->object_id()), "ego", 0.0,
                    (ego_v * ego_target_tangent_cos), 0.0, kEgoFollowTimeBuffer,
                    max_decel_for_front_object, 0.0, dec_str,
                    &leader_max_decel);
