...
absl::Status CheckGeometryGraphConnectivity(const GeometryGraph &graph) {
    ...
    for (const auto &node : nodes) {
        const auto &outgoing_edges_per_node =
            graph.GetOutgoingEdges(node.index);
    ...
    for (int i = 0; i < graph.nodes_layers().size(); ++i) {
        ...
        for (const auto &node_idx : layer) {
            ...
            const auto &outgoing_edges_per_node =
                graph.GetOutgoingEdges(node_idx);
            if (outgoing_edges_per_node.size() > 0) {
                if (!node.reachable) {
                    std::string result = absl::StrFormat(
                        "From nodes_layers: Node %d has non-zero outgoing "
                        "edges but is "
                        "recorded as unreachable. It is resampled? %d. It is "
                        "at layer "
                        "idx: %d",
