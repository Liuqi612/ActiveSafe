...
class StationaryGeometry : public GeometryForm {
 public:
    ...
    GeometryState State(double) const override { return state_[0]; }
    const GeometryState &FastState(double) const override { return state_[0]; }
    std::vector<GeometryState> Sample(double) const override { return state_; }
    absl::Span<const GeometryState> states() const override {
