...
#include "math/cubic_spiral_boundary_value_problem.h"
#include "math/cubic_spline.h"
#include "math/fast_math.h"
#include "math/quintic_spiral_boundary_value_problem.h"
...
std::vector<double> ComputeQuinticPolynomialCoefficients(const double x0,
                                                         const double dx0,
                                                         const double ddx0,
                                                         const double x1,
                                                         const double dx1,
...
double CalculateCurvature(const double &ref_k,
                          const double &dref_k,
                          const double &l,
                          const double &dl,
                          const double &ddl) {
    double denominator = (dl * dl + (1.0 - l * ref_k) * (1.0 - l * ref_k));
    if (std::fabs(denominator) < 1e-8) {
        return 0.0;
    }
    denominator *= std::sqrt(denominator);
    const double numerator = ref_k + ddl - 2.0 * l * ref_k * ref_k -
                             l * ddl * ref_k + l * l * ref_k * ref_k * ref_k +
                             l * dl * dref_k + 2.0 * dl * dl * ref_k;
...
BatchQueryFrenetCoordinateOnDrivePassage(
    absl::Span<const SpiralPoint> spiral_points,
    const FrenetFrame *passage_ff,
    ...
    for (const auto &point : spiral_points) {
        ...
        ASSIGN_OR_RETURN(
            ...
            passage_ff->XYToSLWithHeadingDiffLimit(
                point_xy, point.theta, kFrenetProjectionMaxHeadingDiff),
...
absl::StatusOr<std::vector<GeometryState>> SpiralPoints2States(
    ...
    if (!XCHECK_EQ(frenet_points.size(), spiral_points.size())) {
        return states;
    }
    for (std::size_t i = 0U; i < spiral_points.size(); i++) {
...
SmoothFeature GetSmoothedDrivePassageFeaturesAtS(
    ...
    SmoothFeature features = {
        ...
        .ddk = 0.0,
        .theta = NormalizeAngle(pnc_x::fast_math::Atan2(hsin, hcos))};
...
GeometryFormBuilder::GeometryFormBuilder(const DrivePassage *passage,
    ...
    if (!XCHECK_NOTNULL(passage_)) {
        return;
    }
    init_ = false;

    const int station_size = passage_->stations().size();
    station_k_.reserve(static_cast<std::size_t>(station_size));
    ...
    unsmoothed_xy.reserve(static_cast<std::size_t>(station_size));
    x.reserve(static_cast<std::size_t>(approximate_resample_size));
    y.reserve(static_cast<std::size_t>(approximate_resample_size));
    s.reserve(static_cast<std::size_t>(approximate_resample_size));

    for (const auto &station : passage_->stations()) {
        if ((static_cast<int>(s.size()) > kSmoothDrivePassageMinResampleStationSize) &&
    ...
    constexpr int kMinContainerSize2 = 2;
    if ((static_cast<int>(s.size()) < kMinContainerSize2) || (static_cast<int>(x.size()) < kMinContainerSize2) || (static_cast<int>(y.size()) < kMinContainerSize2)) {
        return;
    }
    ...
    smoothed_xy_.reserve(static_cast<std::size_t>(station_size));
    std::vector<double> hsin;
    std::vector<double> hcos;
    hsin.reserve(static_cast<std::size_t>(station_size));
    hcos.reserve(static_cast<std::size_t>(station_size));
    std::vector<double> s_all;
    s_all.reserve(static_cast<std::size_t>(station_size));
    for (const auto &station : passage_->stations()) {
        ...
        k_temp =
            boost::algorithm::clamp(k_temp, -kCurvatureClip, kCurvatureClip);
        station_k_.push_back(k_temp);
        const auto tan = Vec2d(dx, dy).normalized();
        hsin.push_back(tan.y());
        hcos.push_back(tan.x());
    }
    station_k_[0] = 0.0;
    // DP geometry queries reference curvature by accumulated station s. Keep
    // the sampled curvature table in the same piecewise-linear representation
    // as the smoothed heading tables; otherwise k_s_ remains empty and the
    // first DP spiral edge dereferences an invalid PLF.
    k_s_ = PiecewiseLinearFunction<double, double>(s_all, station_k_);
    smoothed_hsin_s_ =
        PiecewiseLinearFunction<double, double>(s_all, std::move(hsin));
    smoothed_hcos_s_ =
        PiecewiseLinearFunction<double, double>(s_all, std::move(hcos));

    ASSIGN_OR_DIE(auto unsmoothed_frame,
                  BuildKdTreeFrenetFrame(unsmoothed_xy, true));
    frenet_frame_ =
        std::make_unique<KdTreeFrenetFrame>(std::move(unsmoothed_frame));

    ASSIGN_OR_DIE(auto smoothed_frame,
                  BuildKdTreeFrenetFrame(smoothed_xy_, true));
...
GeometryFormBuilder::BuildLateralQuinticPolyGeometry(
    ...
    std::vector<GeometryState> states;
    states.reserve(static_cast<std::size_t>(discretization_size));
    ...
    for (int i = 0; i < discretization_size; ++i) {
        ...
        ASSIGN_OR_RETURN(const auto point_sl_drive_passage,
                         passage_->QueryFrenetCoordinateAt(point_xy),
                         _ << "Cannot project current point back onto original "
                              "drive passage.");
        ...
        const double k =
            CalculateCurvature(ref_k, dref_k, lateral_offset, dl, ddl);
        ...
        const double angle = NormalizeAngle(
            pnc_x::fast_math::Atan2(hsin, hcos) +
            pnc_x::fast_math::Atan2(dl, 1.0 - ref_k * lateral_offset));
    ...
    states[static_cast<std::size_t>(last_idx)].xy = end.xy;
...
GeometryFormBuilder::BuildCubicSpiralGeometry(
    ...
    const auto &end_station =
        passage_->station(StationIndex(end.station_index));
    const SpiralPoint sp0 = {
        .x = start.xy.x(),
        .y = start.xy.y(),
        .theta = start_station.tangent().FastAngle(),
        .k = SafeCurvatureOffset(station_k_[static_cast<std::size_t>(start.station_index)], start.l),
        .s = 0.0};
    const SpiralPoint sp1 = {
        .x = end.xy.x(),
        .y = end.xy.y(),
        .theta = end_station.tangent().FastAngle(),
        .k = SafeCurvatureOffset(station_k_[static_cast<std::size_t>(end.station_index)], end.l),
        .s = 0.0};
    ...
    const int last_idx = states.size() - 1;
    states[static_cast<std::size_t>(last_idx)].xy = end.xy;
    states[static_cast<std::size_t>(last_idx)].h = NormalizeAngle(sp1.theta);
    states[static_cast<std::size_t>(last_idx)].k = sp1.k;
    states[static_cast<std::size_t>(last_idx)].accumulated_s = end.accumulated_s;
    states[static_cast<std::size_t>(last_idx)].l = end.l;

    XCHECK_GE(states.size(), 2);
    return PiecewiseLinearGeometry(states);
}
...
void GeometryFormBuilder::FillSmoothDrivePassage(
    ...
    smoothed_dp->Clear();
    smoothed_dp->Reserve(static_cast<int>(smoothed_xy_.size()));
