...
#include "initializer/brute_force_collision_checker.h"
...
std::vector<Box2d> StatesToBoxes(absl::Span<const StateType> states,
    ...
    for (const auto &state : states) {
        ...
        boxes.push_back(
            Box2d(center, tangent, params->length(), params->width()));
    }
    return boxes;
}
...
void CheckStationaryObjectCollision(
    ...
    for (const auto *traj : stationary_trajs) {
        ...
        for (int i = 0; i < static_cast<int>(av_boxes.size()); ++i) {
...
void BruteForceCollisionChecker::CheckCollisionWithTrajectories(
    ...
    for (const auto &sampled_traj : sampled_trajs_) {
        ...
        for (int i = 0, t = start_ix;
             (i < static_cast<int>(states.size())) && (t < static_cast<int>(obj_states.size())); ++i, ++t) {
            ...
            if (av_boxes[i].HasOverlap(obj_box)) {
                boost::optional<CollisionConfiguration> config = boost::none;
                if (t > 0) {
                    config = DetermineConfiguration(av_boxes[i],
                                                    obj_states[t - 1].box);
                }
                info->collision_objects.push_back({
                    .time = t * sample_step_,
...
void BruteForceCollisionChecker::CheckCollisionWithStationaryObjects(
    ...
    CheckStationaryObjectCollision(1, states, considered_stationary_trajs_,
                                   vehicle_geom_, stationary_buffer_, info);
}
