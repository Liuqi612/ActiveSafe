...
ReferenceLineSearcherOutput::EdgeInfo ComputeEdgeCost(
    ...
    cost_provider->ComputeRefLineCost(
        edge.geometry, terminating,
        absl::MakeSpan(edge_cost_info.feature_costs));
...
std::vector<Vec2d> GenerateRefLinePoints(
    ...
    for (const auto &edge_idx : edges_list) {
        if (edge_idx == static_cast<GeometryEdgeIndex>(-1)) continue;
...
absl::StatusOr<ReferenceLineSearcherOutput> SearchReferenceLine(
    ...
    if (!XCHECK_NOTNULL(input.geometry_graph) ||
        !XCHECK_NOTNULL(input.drive_passage) ||
        !XCHECK_NOTNULL(input.sl_boundary) ||
        !XCHECK_NOTNULL(input.initializer_params) ||
    ...
    for (int layer_idx = nodes_layers.size() - 1; layer_idx >= 0; --layer_idx) {
        ...
        const auto &nodes_on_cur_layer = nodes_layers[static_cast<size_t>(layer_idx)];
        ...
        for (const auto &node_idx : nodes_on_cur_layer) {
            ...
            for (const auto &edge_idx : outgoing_edges) {
                ...
                const bool terminating =
                    graph.GetOutgoingEdges(edge.end).empty();
                cost_edges[edge_idx] =
                ...
                if (std::isinf(prev_node_cost_info.min_cost)) {
                    ...
                    //         << edge_idx.value()
                    //         << "edge_sum_cost:" << edge_sum_cost;
                    return absl::InternalError(
                        "prev_node_cost infinity, initialization on geometry "
                        "node cost "
                        "might have failed!");
                }
                ...
                if (cost_through_this_edge < cost_nodes[node_idx].min_cost) {
                    // VLOG(5) << "prev_node: " << edge.end.value()
                    //         << "has min_cost" << prev_node_cost_info.min_cost;
                    // VLOG(5)
                    //     << "cost through this edge: " << cost_through_this_edge;
                    // VLOG(5) << "cur node min cost : "
                    //         << cost_nodes[node_idx].min_cost << " so update.";
    ...
    RETURN_IF_ERROR(BuildConnection(graph, cost_nodes, cost_edges,
                                    *cost_provider, &nodes_list, &edges_list,
                                    &feature_costs));
...
absl::Status DeactivateFarGeometries(
    ...
    ASSIGN_OR_RETURN(const auto ref_frame,
                     BuildBruteForceFrenetFrame(downsampled_ref_points, true),
...
absl::Status DeactivateFarGeometries(
    ...
    ASSIGN_OR_RETURN(auto ref_frame,
                     BuildBruteForceFrenetFrame(ref_points, true),
                     _ << "Failed to build frenet frame from ref line.");

    constexpr double kSampleRadiusInterval = 5.0;
    ...
    for (int i = 0; i < radius_anchor_num; ++i) {
        ...
        auto boundary_l = path_sl.QueryBoundaryL(accum_s[i]);
        activate_r[i] = 0.5 * kActivationBoundaryWidthRatioPlf(accum_s[i]) *
                        (boundary_l.second - boundary_l.first);
    }
    PiecewiseLinearFunction<double, double> accum_s_r_plf(accum_s, activate_r);
