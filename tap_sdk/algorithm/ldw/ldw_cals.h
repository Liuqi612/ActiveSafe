#ifndef TAP_SDK_ALGORITHM_LDW_LDW_CALS_H_
#define TAP_SDK_ALGORITHM_LDW_LDW_CALS_H_
...
struct LdwCal {
    ...
    float k_brake_pressure_inhibit_pa = 1200.0F;
    ...
    std::array<float, 7> k_LDW_ovrd_tq_thd_per_spd_x{40.0F, 60.0F, 84.0F, 108.0F, 132.0F, 156.0F, 180.0F}; // 手力矩接管阈值查表的仪表车速轴，单位：km/h
    std::array<float, 7> k_LDW_ovrd_tq_thd_per_spd_z{1.0F, 1.5F, 1.56F, 1.66F, 1.72F, 1.76F, 1.80F}; // 各车速点对应的驾驶员手力矩接管阈值，单位：Nm
    float k_LDW_DriHandsOvr_OnDlyTime = 0.02F; // 手力矩历史条件成立后进入接管抑制的延时，单位：s
    float k_LDW_DriHandsOvr_OffDlyTime = 1.0F; // 手力矩历史条件解除后保持接管抑制的延时，单位：s
    uint8_t k_LDW_DriHandsOvr_HistoryWindow = 10U; // 手力矩接管判定的历史统计窗口，单位：帧
    uint8_t k_LDW_DriHandsOvr_HistorySuppressCount = 6U; // 历史窗口内达到手力矩阈值即可判定接管的最少帧数，单位：帧
    ...
    float k_warning_boundary_maxdist_m = 0.25F;
    float k_warning_boundary_mindist_m = -0.15F;
    float k_left_boundary_lat_rate_mps = 0.15F;
    float k_right_boundary_lat_rate_mps = -0.15F;
    float k_left_boundary_heading_deg = 1.0F;
    float k_right_boundary_heading_deg = -1.0F;
    float k_warning_ttlc_s = 0.9F;
    float k_low_speed_warning_ttlc_s = 0.65F; // 低速区间的 LDW 告警 TTLC 基础门限，单位：s
    ...
    float k_left_lane_offset_m = -0.10F;
    float k_right_lane_offset_m = 0.10F;

    float k_warning_min_hold_s = 2.0F;
    float k_warning_max_hold_s = 7.0F;

    float k_low_sensitivity_scale = 0.85F;
    float k_high_sensitivity_scale = 1.15F;

    uint16_t k_standby_enable_mask = 0xFFU;    // bit0-7: 进入 Enable 前要求全部满足
    uint32_t k_warning_suppressed_mask = 0xE00U; // bit9: 低速油门，bit10: 手力矩接管，bit11: 综合驾驶意图
};

} // namespace ldw
} // namespace active_safety
#endif // TAP_SDK_ALGORITHM_LDW_LDW_CALS_H_
