...
ElkFunction::ElkFunction() {
    Init();
}

void ElkFunction::Init() {
...
void ElkFunction::Update(const AsVseOut &vse_out, const LanesInfo &road_info, const GlobalConfig &config, const AsObstacleList &obs_list) {
    ...
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        ...
        if (obs_data_[i].obs && (obs_data_[i].obs->status == TrackStatus::NEW || obs_data_[i].obs->status == TrackStatus::INVALID)) {
            obs_data_[i].Reset();
            obs_data_[i].obs = obs_list[i];
        }
    }
...
void ElkFunction::OncomTrkPropertiesUpdate(const LanesInfo &lane_info, float rear_curvature) {
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        // Following as_elk: updateTrkOncomConfidence uses prev_* from last frame
        UpdateTrkOncomConfidence(i);
        
        // Following as_elk: updatePrevLctProperties updates prev_* for next frame
...
void ElkFunction::OvertakeTrkPropertiesUpdate(const LanesInfo &lane_info, float rear_curvature) {
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
...
void ElkFunction::UpdateTrkOncomConfidence(size_t obs_idx) {
    ...
    bool f_available_source = (obs->fusion_source == FusionSource::RADAR_VISION ||
                               obs->fusion_source == FusionSource::MULTIPLE_TRACKLET);
    bool f_low_conf = false;  // Following as_elk: always false
    bool f_high_conf = true;  // Following as_elk: always true
    bool f_available_motion = (obs->long_vel < 0.01f);
    bool f_available_heading = (obs->heading > elk_cal_.k_trk_heading || obs->heading < -elk_cal_.k_trk_heading);
    ...
    switch (memory.oncom_confidence) {
        ...
        case Confidence::MED:
            if (memory.cnt_not_available_object_class_age > 3 || !f_available_status || memory.cnt_not_oncoming_age > 3) {
                ...
                memory.oncom_age = 1;
            } else if (f_available_class && !f_object_class_change && !f_visionID_change && f_available_source && 
                      memory.oncom_age > 3 && f_available_motion && f_available_heading && f_high_conf) {
                memory.oncom_confidence = Confidence::HIGH;
                memory.oncom_age = 1;
            } else {
                memory.oncom_age = std::min((uint8_t)255, (uint8_t)(memory.oncom_age + 1));
...
void ElkFunction::UpdateTrkOvertakeConfidence(size_t obs_idx) {
    // Overtake confidence is not used in as_elk, keeping for compatibility
}

// Following as_elk 100%: CheckOncomValidZone
void ElkFunction::CheckOncomValidZone(size_t obs_idx) {
    auto &obs_data = obs_data_[obs_idx];
    auto &memory = obs_data.persistent;
    
    float tmp = (obs_data.obs->object_class == ObjectClass::TRUCK) ? 
                elk_cal_.k_neighbor_factor_truck : elk_cal_.k_neighbor_factor;
    tmp *= oncom_persistents_.LM_properties.host_lane_width;
    
    memory.f_zone_interest = (obs_data.obs->long_posn > elk_cal_.k_oc_min_x && 
                             obs_data.obs->long_posn < elk_cal_.k_oncom_long_thres && 
                             std::abs(obs_data.dist) < tmp);
}

// Following as_elk 100%: CheckOvertakeValidZone
void ElkFunction::CheckOvertakeValidZone(size_t obs_idx) {
    auto &obs_data = obs_data_[obs_idx];
    auto &memory = obs_data.persistent;
    ...
    memory.f_rear_zone_interest = (obs_data.obs->long_posn > elk_cal_.k_ot_rear_min_x && 
                                   obs_data.obs->long_posn < elk_cal_.k_ot_rear_max_x && 
                                   std::abs(obs_data.dist) < tmp);
    
    memory.f_front_zone_interest = (obs_data.obs->long_posn > elk_cal_.k_ot_front_min_x && 
                                    obs_data.obs->long_posn < elk_cal_.k_ot_front_max_x && 
                                    std::abs(obs_data.dist) < tmp);
}

// Following as_elk 100%: OncomThreatComputation
void ElkFunction::OncomThreatComputation(float host_speed) {
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        auto &obs_data = obs_data_[i];
        auto &memory = obs_data.persistent;
        
        if (memory.oncom_confidence > Confidence::LOW && 
            memory.f_zone_interest && 
            !obs_data.f_beyond_border) {
...
void ElkFunction::OvertakeThreatComputation(float host_speed) {
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        auto &obs_data = obs_data_[i];
        
        if (!obs_data.f_beyond_border && obs_data.obs->track_id > 0) {
            float dv = host_speed - obs_data.obs->long_vel;
            float ttc = 99.0f;
            if (std::abs(dv) > 0.001f) {
                ttc = std::abs(obs_data.obs->long_posn / dv);
...
void ElkFunction::ProcessLaneMarkers(const LanesInfo &lane_info) {
    // This function is now replaced by CheckLaneMarkerValid
    // Keeping for backward compatibility
}

void ElkFunction::ProcessObjects(const AsVseOut &vse_out, const LanesInfo &road_info, const AsObstacleList &obs_list) {
...
void ElkFunction::UpdateLaneAssignment(size_t obs_idx, const LanesInfo &lane_info, float rear_curvature) {
    auto &obs_data = obs_data_[obs_idx];
    ...
    float offset_left = -lane_info.HostLeftLaneMarker.LinePolyC0 + 
                       (-lane_info.HostLeftLaneMarker.LinePolyC1) * trk_vcs_long_posn +
                       (-lane_info.HostLeftLaneMarker.LinePolyC2) * x2 + 
                       (-lane_info.HostLeftLaneMarker.LinePolyC3) * x2 * trk_vcs_long_posn;

    float offset_right = -lane_info.HostRightLaneMarker.LinePolyC0 + 
                        (-lane_info.HostRightLaneMarker.LinePolyC1) * trk_vcs_long_posn +
                        (-lane_info.HostRightLaneMarker.LinePolyC2) * x2 + 
                        (-lane_info.HostRightLaneMarker.LinePolyC3) * x2 * trk_vcs_long_posn;
...
void ElkFunction::OncomTargetSelection(const AsVseOut &vse_out) {
...
void ElkFunction::OvertakeTargetSelection(const AsVseOut &vse_out) {
    // This function is now replaced by the flow in Update()
    // Keeping for backward compatibility
}

// Following as_elk 100%: FillOncomOutput (targets are already selected in OncomSelect)
void ElkFunction::FillOncomOutput() {
...
void ElkFunction::FillOvertakeOutput() {
...
void ElkFunction::FillTargetInfo(const ElkObsData &obs_data, AsOnComingObject &oncom_tgt) {
...
// Note: Convert coordinate system from ISO (left+, right-) to as_elk (left-, right+)
void ElkFunction::FillTargetInfo(const ElkObsData &obs_data, AsOverTakingObject &overtake_tgt) {
    const auto &obs = obs_data.obs;
...
void ElkFunction::UpdateOncomCandidatePool() {
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_left = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    oncom_persistents_.candidate_pool_ID_left.fill(0);
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_right = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    oncom_persistents_.candidate_pool_ID_right.fill(0);
    ...
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        ...
        if (obs_data_[i].collision_prob > CollisionProbability::NONE) {
            // Left pool
            if (!IsMemberOfArray(track_id, oncom_persistents_.oncom_left_id_pool) &&
                obs_data_[i].persistent.oncom_confidence > Confidence::LOW &&
            ...
            // Right pool
            if (!IsMemberOfArray(track_id, oncom_persistents_.oncom_right_id_pool) &&
                obs_data_[i].persistent.oncom_confidence > Confidence::LOW &&
...
void ElkFunction::UpdateOvertakeCandidatePool() {
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_rear_left = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    overtake_persistents_.candidate_pool_ID_rear_left.fill(0);
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_rear_right = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    overtake_persistents_.candidate_pool_ID_rear_right.fill(0);
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_front_left = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    overtake_persistents_.candidate_pool_ID_front_left.fill(0);
    ...
    std::array<CollisionProbability, 3> candidate_collision_prob_pool_front_right = {CollisionProbability::NONE, CollisionProbability::NONE, CollisionProbability::NONE};
    overtake_persistents_.candidate_pool_ID_front_right.fill(0);
    ...
    for (size_t i = 0; i < kMaxNormalObject; ++i) {
        ...
        if (obs_data_[i].collision_prob > CollisionProbability::NONE) {
            // Rear left
            if (!IsMemberOfArray(track_id, overtake_persistents_.overtake_rear_left_id_pool) &&
                obs_data_[i].lane == ElkLane::LEFT &&
            ...
            // Rear right
            if (!IsMemberOfArray(track_id, overtake_persistents_.overtake_rear_right_id_pool) &&
                obs_data_[i].lane == ElkLane::RIGHT &&
                obs_data_[i].persistent.f_rear_zone_interest) {
                ...
                if (j_rear_right > elk_cal_.k_Max_Overtake_Pool_Trks) {
                    RankCandidate(overtake_persistents_.candidate_pool_ID_rear_right, candidate_pool_long_posn_rear_right, candidate_collision_prob_pool_rear_right);
            ...
            // Front left
            if (!IsMemberOfArray(track_id, overtake_persistents_.overtake_front_left_id_pool) &&
                obs_data_[i].lane == ElkLane::LEFT &&
                ...
                if (j_front_left > elk_cal_.k_Max_Overtake_Pool_Trks) {
                    RankCandidate(overtake_persistents_.candidate_pool_ID_front_left, candidate_pool_long_posn_front_left, candidate_collision_prob_pool_front_left);
            ...
            // Front right
            if (!IsMemberOfArray(track_id, overtake_persistents_.overtake_front_right_id_pool) &&
                obs_data_[i].lane == ElkLane::RIGHT &&
                obs_data_[i].persistent.f_front_zone_interest) {
                ...
                if (j_front_right > elk_cal_.k_Max_Overtake_Pool_Trks) {
                    RankCandidate(overtake_persistents_.candidate_pool_ID_front_right, candidate_pool_long_posn_front_right, candidate_collision_prob_pool_front_right);
...
uint8_t ElkFunction::UpdateOncomTargetPool(const std::array<uint8_t, 3> &candidate_pool_ID, ElkLane laneEnum) {
    std::array<uint8_t, 3> &oncom_id_pool = (laneEnum == ElkLane::LEFT) ? 
                                            oncom_persistents_.oncom_left_id_pool : 
                                            oncom_persistents_.oncom_right_id_pool;
        ...
        if (track_ID > 0) {
            ...
            if (obs_idx < kMaxNormalObject) {
                if (obs_data_[obs_idx].persistent.oncom_confidence == Confidence::LOW ||
                    obs_data_[obs_idx].collision_prob == CollisionProbability::NONE ||
                    obs_data_[obs_idx].lane != laneEnum) {
...
uint8_t ElkFunction::UpdateOvertakeTargetPool(const std::array<uint8_t, 3> &candidate_pool_ID, 
                                               bool frontFlag, bool rearFlag, ElkLane laneEnum,
        ...
        if (track_ID > 0) {
            ...
            if (obs_idx < kMaxNormalObject) {
                bool b_vld_long_zone = ((obs_data_[obs_idx].persistent.f_front_zone_interest == frontFlag) &&
                                      ...
                                      (obs_data_[obs_idx].persistent.f_front_zone_interest && 
                                       obs_data_[obs_idx].persistent.f_rear_zone_interest);
...
void ElkFunction::ReArrangePool(const std::array<uint8_t, 3> &extra_id, uint8_t extra_id_valid_length,
                                 std::array<uint8_t, 3> &id_pool) {
    ...
    uint8_t max_loop = (elk_cal_.k_Max_Oncom_Pool_Trks > pool_size) ? pool_size : elk_cal_.k_Max_Oncom_Pool_Trks;
    ...
    for (uint8_t i = 0; i < extra_id_valid_length; ++i) {
        ...
        if (candidate_id == 0 || candidate_id > kMaxNormalObject) continue;
        size_t candidate_idx = candidate_id - 1;
        
        for (uint8_t j = 0; j < avail_id_valid_length; ++j) {
            uint8_t track_id = avail_id[j];
            if (track_id == 0 || track_id > kMaxNormalObject) continue;
...
void ElkFunction::OncomDownSelection() {
    UpdateOncomCandidatePool();
    
    oncom_persistents_.num_oncom_left_trks = UpdateOncomTargetPool(
        oncom_persistents_.candidate_pool_ID_left, ElkLane::LEFT);
    
    oncom_persistents_.num_oncom_right_trks = UpdateOncomTargetPool(
        oncom_persistents_.candidate_pool_ID_right, ElkLane::RIGHT);
}

// Following as_elk 100%: UpdateOvertakePool
void ElkFunction::UpdateOvertakePool() {
    overtake_persistents_.num_overtake_rear_left_trks = UpdateOvertakeTargetPool(
        overtake_persistents_.candidate_pool_ID_rear_left, false, true, ElkLane::LEFT,
        overtake_persistents_.overtake_rear_left_id_pool);
    
    overtake_persistents_.num_overtake_rear_right_trks = UpdateOvertakeTargetPool(
        overtake_persistents_.candidate_pool_ID_rear_right, false, true, ElkLane::RIGHT,
        overtake_persistents_.overtake_rear_right_id_pool);
    
    overtake_persistents_.num_overtake_front_left_trks = UpdateOvertakeTargetPool(
        overtake_persistents_.candidate_pool_ID_front_left, true, false, ElkLane::LEFT,
        overtake_persistents_.overtake_front_left_id_pool);
    
    overtake_persistents_.num_overtake_front_right_trks = UpdateOvertakeTargetPool(
        overtake_persistents_.candidate_pool_ID_front_right, true, false, ElkLane::RIGHT,
        overtake_persistents_.overtake_front_right_id_pool);
}

// Following as_elk 100%: OvertakeDownSelection
void ElkFunction::OvertakeDownSelection() {
    UpdateOvertakeCandidatePool();
    UpdateOvertakePool();
}

// Following as_elk 100%: OncomSelect
void ElkFunction::OncomSelect() {
...
void ElkFunction::OvertakeSelect() {
...
void ElkFunction::UpdateOncomTgtTrkProperties() {
...
void ElkFunction::UpdateOvertakeTgtTrkProperties() {
    ...
    AsOverTakingObject* targets[4] = {
        &overtake_out_.overtake_target_rear_left,
        &overtake_out_.overtake_target_rear_right,
        &overtake_out_.overtake_target_front_left,
        &overtake_out_.overtake_target_front_right
    };
...
void ElkFunction::CheckLaneMarkerValid(const LanesInfo &lane_info) {
    // Convert from ISO (left+, right-) to as_elk (left-, right+)
    ...
    oncom_persistents_.LM_properties.f_host_left = 
        (lane_info.HostLeftLaneMarker.LineConf >= elk_cal_.k_lineconf_vld_thr) && 
        (lane_info.HostLeftLaneMarker.LineViewRangeEnd > elk_cal_.k_range_lm) &&
        (left_c0 <= elk_cal_.k_offset_small) &&
        (left_c0 >= -elk_cal_.k_offset_big);

    oncom_persistents_.LM_properties.f_host_right = 
        (lane_info.HostRightLaneMarker.LineConf >= elk_cal_.k_lineconf_vld_thr) && 
        (lane_info.HostRightLaneMarker.LineViewRangeEnd > elk_cal_.k_range_lm) &&
        (right_c0 >= -elk_cal_.k_offset_small) &&
...
// Note: Input coordinate system is ISO (left positive, right negative)
// as_elk expects (left negative, right positive), so we need to negate the values
void ElkFunction::UpdateHostDepartureDirection(const LanesInfo &lane_info) {
    ...
    } else if (oncom_persistents_.LM_properties.f_host_left &&
               oncom_persistents_.LM_properties.f_host_right) {
...
void ElkFunction::CheckObjectBeyondBarrier(size_t obs_idx, const LanesInfo &lane_info) {
    auto &obs_data = obs_data_[obs_idx];
    // Convert object position from ISO (left+, right-) to as_elk (left-, right+)
    float trk_vcs_lat_posn = -obs_data.obs->lat_posn;  // Negate to convert coordinate system
    float trk_vcs_long_posn = obs_data.obs->long_posn; // Longitudinal position doesn't change
    ...
    if ((trk_vcs_long_posn < lane_info.LeftRoadEdge.LineViewRangeEnd) &&
        ...
        // Negate all polynomial coefficients to convert coordinate system
        offset_on_edge_left = -lane_info.LeftRoadEdge.LinePolyC0 + 
                              (-lane_info.LeftRoadEdge.LinePolyC1) * trk_vcs_long_posn +
                              (-lane_info.LeftRoadEdge.LinePolyC2) * x2 + 
                              (-lane_info.LeftRoadEdge.LinePolyC3) * x2 * trk_vcs_long_posn;
    }
    ...
    if ((trk_vcs_long_posn < lane_info.RightRoadEdge.LineViewRangeEnd) &&
        ...
        // Negate all polynomial coefficients to convert coordinate system
        offset_on_edge_right = -lane_info.RightRoadEdge.LinePolyC0 + 
                               (-lane_info.RightRoadEdge.LinePolyC1) * trk_vcs_long_posn +
                               (-lane_info.RightRoadEdge.LinePolyC2) * x2 + 
                               (-lane_info.RightRoadEdge.LinePolyC3) * x2 * trk_vcs_long_posn;
    }

    if ((trk_vcs_lat_posn < offset_on_edge_left) || 
        (trk_vcs_lat_posn > offset_on_edge_right)) {
...
uint8_t ElkFunction::FindMinIndex(const std::array<float, 3> &array, uint8_t startIdx, uint8_t endIdx) {
    uint8_t min_indx = 1;
...
bool ElkFunction::IsMemberOfArray(uint8_t val, const std::array<uint8_t, 3> &array) {
    for (uint8_t i = 0; i < 3; ++i) {
        if (val == array[i]) {
            return true;
        }
...
void ElkFunction::RankCandidate(std::array<uint8_t, 3> &candidate_pool_ID, 
                                std::array<float, 3> &candidate_pool_long_posn,
                                std::array<CollisionProbability, 3> &candidate_collision_prob_pool) {
    uint8_t num_candidate = 0;
    for (uint8_t k = 0; k < 3; ++k) {
        if (candidate_pool_ID[k] > 0) num_candidate++;
    }
