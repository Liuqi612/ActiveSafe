...
class ElkFunction {
  ...
  private:
    ...
    void RankCandidate(std::array<uint8_t, 3> &candidate_pool_ID, 
    ...
    uint8_t UpdateOvertakeTargetPool(const std::array<uint8_t, 3> &candidate_pool_ID, 
                                     bool frontFlag, bool rearFlag, ElkLane laneEnum,
                                     std::array<uint8_t, 3> &id_pool);
    void ReArrangePool(const std::array<uint8_t, 3> &extra_id, uint8_t extra_id_valid_length,
                       std::array<uint8_t, 3> &id_pool);
