...
EgoMotionStateAtTime AsEgoPath::GetHostStateAtTime(float time) {
    ...
    if (SpeedValid) {
        ...
        for (std::size_t i = 0; i < 4; i++) {
            dx_2[i]      = -3.0 * path_estimn[i].ctrl_point_long_p0 + 3.0 * path_estimn[i].ctrl_point_long_p1;
            dy_2[i]      = -3.0 * path_estimn[i].ctrl_point_lat_p0 + 3.0 * path_estimn[i].ctrl_point_lat_p1;
...
void AsEgoPath::UpdatePath(float vlgt, float algt, float c0, float c1, float width, float length, float distRearToFnt) {
    ...
    for (std::size_t idx = 1; idx <= 4; idx++) {
        ...
        if (std::abs(t0.speed) >= spd_thres_in_evaluation) {
            ...
            longitudinal_stopping_dist = t0.vel_long * stop_time + 0.5 * t0.accel_long * stop_time * stop_time;
            lateral_stopping_dist      = t0.vel_lat * stop_time + 0.5 * t0.accel_lat * stop_time * stop_time;
        }
        ...
        if (std::abs(t0.curvature) > detectableCuvature && std::abs(t0.curvature + t0.curvature_rate) > detectableCuvature) {
            ...
            float distance = 0.5 * t0.accel + t0.speed;
            // Curvature radius.
            ...
            path_estimn[idx - 1].ctrl_point_long_p3 = t0.position_long + (0.5 * t0.accel + t0.speed) * std::cos(t0.heading_angle);
            path_estimn[idx - 1].ctrl_point_lat_p3  = t0.position_lat + (0.5 * t0.accel + t0.speed) * std::sin(t0.heading_angle);
        }
        ...
        // c = c0 + c1 * 1s.
        t1.curvature              = math::Clamp(t0.curvature + t0.curvature_rate, -0.1, 0.1);
