...
void LgSafe2024Function::AbortAebCmd() {
    ...
    cmd_output_.aeb_req = senseAD::tap::OnOffStatus::NotActive;
    cmd_output_.aeb_tar_dec = float(0);
    cmd_output_.aeb_req_valid = uint8_T(0);
    cmd_output_.awb_req = senseAD::tap::OnOffStatus::NotActive;
    cmd_output_.awb_level = uint8_T(0);
    if (cmd_output_.aeb_status == senseAD::tap::AebStatus::AEB_ACTIVE) {

        cmd_output_.aeb_status = senseAD::tap::AebStatus::AEB_Inhibit;
    }
}
void LgSafe2024Function::SetAebUnavoidCollision(const uint8_T unavoid_collision_flag) {

    cmd_output_.resv1 = static_cast<uint32_T>(unavoid_collision_flag);
}

void LgSafe2024Function::SetFcwThreatFlag(const uint8_T fcw_threat_flag) {

    cmd_output_.resv2 = fcw_threat_flag > 0 ? 1 : 0;
}

void LgSafe2024Function::SetAebThreatFlag(const uint8_T aeb_threat_flag) {

    cmd_output_.resv3 = aeb_threat_flag > 0 ? 1 : 0;
}
void LgSafe2024Function::Update(const active_safety::AsVseOut &vse_in, const active_safety::AsObstacleList &obs_list,
                                const active_safety::LanesInfo &road_info, const senseAD::tap::AsParamConfig_T &long_safe_param,
                                const uint8_T aeb_fcw_exit_inhibit,const GlobalConfig &config) {
    senseAD::tap::AsVseOut vse_out;
    MapVseInput(vse_in, vse_out,config);

    senseAD::tap::AS_LaneMarkerInfo_T lane_info;
    MapRoadInfo(road_info, lane_info);

    society_scene_.ProcessSocietyScene(obs_list, vse_out, long_safe_param);
    select_module_.Run(obs_list, lane_info, vse_out, society_scene_);
    select_module_.GetLongSafeOut(select_output_, society_scene_);
...
void LgSafe2024Function::MapNewTselRes2SelectOut(senseAD::tap::AsLongSafeObject &res, active_safety::longsafe::LongSafeObject lgsf_obj) {
    ...
    res.long_posn = lgsf_obj.long_posn - 3.7;
    ...
    if ((lgsf_obj.type == ObjectClass::PEDESTRIAN)  ||  (lgsf_obj.type == ObjectClass::CONE)  ||  (lgsf_obj.type == ObjectClass::GOD)  ||  (lgsf_obj.type == ObjectClass::OCC)) {

        res.lat_posn = lgsf_obj.lat_posn;
        res.long_posn = lgsf_obj.long_posn - 3.7;
    } else {
        res.lat_posn = lgsf_obj.lat_posn - 0.5F * sinf(lgsf_obj.heading) * lgsf_obj.length;
        res.long_posn = lgsf_obj.long_posn - 0.5F * cosf(lgsf_obj.heading) * lgsf_obj.length - 3.7;
    }
...
void LgSafe2024Function::MapToSimulinkInput(const senseAD::tap::AsVseOut &vse_out, const senseAD::tap::AsLongSafeOut &select_out,
    ...
    if(abs(simulink_u_core_out_.longsafe.longsafe_aeb.heading)> 1.4 &&abs(simulink_u_core_out_.longsafe.longsafe_aeb.heading)< 1.75 &&
           simulink_u_core_out_.longsafe.longsafe_aeb.stationary && simulink_u_core_out_.longsafe.longsafe_aeb.object_class == 9 &&
       abs(simulink_u_core_out_.longsafe.longsafe_aeb.lat_posn) < 1.2 && simulink_u_core_out_.longsafe.longsafe_aeb.age > 0 &&
       simulink_u_core_out_.longsafe.longsafe_aeb.heading * simulink_u_core_out_.longsafe.longsafe_aeb.lat_posn < 0){
           simulink_u_core_out_.longsafe.longsafe_aeb.lat_posn = 0.35;
           }
    if(abs(simulink_u_core_out_.longsafe.longsafe_fcw.heading)> 1.4 &&abs(simulink_u_core_out_.longsafe.longsafe_fcw.heading)< 1.75 &&
           simulink_u_core_out_.longsafe.longsafe_fcw.stationary && simulink_u_core_out_.longsafe.longsafe_fcw.object_class == 9 &&
        abs(simulink_u_core_out_.longsafe.longsafe_aeb.lat_posn) < 1.2 && simulink_u_core_out_.longsafe.longsafe_aeb.age > 0 &&
        simulink_u_core_out_.longsafe.longsafe_fcw.heading * simulink_u_core_out_.longsafe.longsafe_aeb.lat_posn < 0){
           simulink_u_core_out_.longsafe.longsafe_fcw.lat_posn = 0.35;
    }
...
void LgSafe2024Function::MapFromSimulinkOutput(const senseAD::tap::LgSafe_T &lg_safe_internal, const senseAD::tap::AsCmdLgSafe_T &cmd_internal,
    ...
    if (vse_out.speed > long_safe_param.k_AEB_FullBrkSpdThres) {
        cmd_output_.aeb_tar_dec = fmax(cmd_output_.aeb_tar_dec, long_safe_param.k_AEB_HighReqDecel);
    }
...
void LgSafe2024Function::MapVseInput(const active_safety::AsVseOut &vse_in, senseAD::tap::AsVseOut &vse_out,const GlobalConfig &config) {
    ...
    if(config.k_Lgsf_ForOldRsclBag){
        vse_out.aeb_switch           = 1;
        vse_out.fcw_switch           = 1;
        vse_out.fcw_sensitivity_set  = 2;
    }else{
        vse_out.aeb_switch           = vse_in.aeb_switch;
        vse_out.fcw_switch           = vse_in.fcw_switch;
        vse_out.fcw_sensitivity_set  = vse_in.fcw_sensitivity_set;
    }
...
void LgSafe2024Function::MapRoadInfo(const active_safety::LanesInfo &road_info_in, senseAD::tap::AS_LaneMarkerInfo_T &lane_info_out) {
    lane_info_out.LaneMarkerHeader.LineObjectsTimeStampSec = static_cast<uint32_t>(road_info_in.time / 1000000000ULL);
    lane_info_out.LaneMarkerHeader.LineObjectsTimeStampNSec = static_cast<uint32_t>(road_info_in.time % 1000000000ULL);

    MapLaneMarker(road_info_in.HostLeftLaneMarker, lane_info_out.HostLeftLaneMarker);
    MapLaneMarker(road_info_in.HostRightLaneMarker, lane_info_out.HostRightLaneMarker);
    MapLaneMarker(road_info_in.LeftLaneLeftLaneMarker, lane_info_out.LeftLaneLeftLaneMarker);
    MapLaneMarker(road_info_in.LeftLaneRightLaneMarker, lane_info_out.LeftLaneRightLaneMarker);
    MapLaneMarker(road_info_in.RightLaneLeftLaneMarker, lane_info_out.RightLaneLeftLaneMarker);
    MapLaneMarker(road_info_in.RightLaneRightLaneMarker, lane_info_out.RightLaneRightLaneMarker);
    MapLaneMarker(road_info_in.RearLaneLeftLaneMarker, lane_info_out.RearLaneLeftLaneMarker);
    MapLaneMarker(road_info_in.RearLaneRightLaneMarker, lane_info_out.RearLaneRightLaneMarker);
    MapRoadEdge(road_info_in.LeftRoadEdge, lane_info_out.LeftRoadEdge);
    MapRoadEdge(road_info_in.RightRoadEdge, lane_info_out.RightRoadEdge);
}

void LgSafe2024Function::MapLaneMarker(const active_safety::LaneMarker_t &in, senseAD::tap::LaneMarker_t &out) {
    out.LineObjectsRollingCounter = in.LineObjectsRollingCounter;
    out.LineValid = in.LineValid;
    out.LineColor = in.LineColor;
    out.LineType = in.LineType;
    out.LineConf = in.LineConf;
    out.LineViewRangeStart = in.LineViewRangeStart;
    out.LineViewRangeEnd = in.LineViewRangeEnd;
    out.LineMakerWidth = in.LineMakerWidth;
    out.LinePolyC0 = in.LinePolyC0;
    out.LinePolyC1 = in.LinePolyC1;
    out.LinePolyC2 = in.LinePolyC2;
    out.LinePolyC3 = in.LinePolyC3;
}

void LgSafe2024Function::MapRoadEdge(const active_safety::RoadEdge_t &in, senseAD::tap::RoadEdge_t &out) {
    out.LineObjectsRollingCounter = in.LineObjectsRollingCounter;
    out.LineValid = in.LineValid;
    out.LineColor = in.LineColor;
    out.LineType = in.LineType;
